...
-define(PACKAGE,
        "<package name=\"~s\" line-rate=\"~f\" branch-rate=\"0\" "
        "complexity=\"0\"><classes>~s</classes></package>").
-define(CLASS,
        "<class name=\"~s\" filename=\"~s\" line-rate=\"~f\" "
...
-export([start_link/0, init_file/3, store/3, store/4, dump/0, get_config/0]).
...
-type statement_type() ::
    print | eval | assign | if_block | switch | switch_case | for | foreach | while |
    {call, binary()} | pre_incr | post_incr | pre_decr | post_decr | {op, Type :: atom()} |
    class | function | global | return | int | float | text | object | define | constant |
    switch_default.
...
-spec init_file(is_cover(), Filename :: binary(), Compiled :: [main_statement()]) -> ok.
...
init_file(true, Filename, Compiled) ->
    case erlang:get(cover) of
        undefined ->
            ok;
        PID ->
            gen_server:cast(PID, {init_file, Filename, Compiled})
    end.
...
store(_Type, File, Pos) ->
    case erlang:get(cover) of
        undefined ->
            ok;
        PID ->
            gen_server:cast(PID, {store, File, Pos})
    end.
...
handle_cast({init_file, Filename, Compiled}, Files) ->
    case lists:keyfind(Filename, 1, Files) of
        ...
        false ->
            ...
            FileDict = lists:foldl(fun(N, D) -> orddict:store(N, 0, D) end, orddict:new(), Lines),
...
handle_cast({store, File, Pos}, Files) ->
    ...
    NewFiles =
        orddict:update(File, fun(Dict) -> orddict:update_counter(Line, 1, Dict) end, [], Files),
...
handle_call(dump, _From, Files) ->
    ...
    Sources = lists:map(fun({File, _}) -> io_lib:format(?SOURCE, [dirname(File)]) end, Files),
    Epoch = ephp_datetime:posix_time_ms(),
    XML = io_lib:format(?XML_HEAD, [Epoch, Percentage, Sources, Packages]),
    Output =
        case ephp_config:get(<<"cover.output">>) of
            undefined ->
                <<"cobertura.xml">>;
            Filename ->
                Filename
...
-spec process(Compiled :: [main_statement()], Lines :: cover_dict()) -> cover_dict().
...
dump([{File, Dict} | Cover], Classes) ->
    ...
    Lines = lists:flatmap(fun({Line, Hits}) -> io_lib:format(?ENTRY, [Line, Hits]) end, Dict),
...
total_percentage(Files) ->
    {A, T} =
        lists:foldl(fun({_, Dict}, {A, T}) -> {A + get_active_lines(Dict), T + length(Dict)} end,
                    {0, 0},
                    Files),
    A / T.
