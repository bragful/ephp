...
-export([start_link/0, destroy/1, get/3, get/4, get_constructor/2, get_destructor/2,
         get_attribute/2, get_clone/2, get_method/2, get_method/3, get_parent/3, class_attr/1,
         class_attr/2, class_attr/3, class_attr/4, init_static_value/6, set_static/6,
         register_class/4, register_classes/2, register_classes/3, register_interface/3,
         set_alias/5, instance/6, get_stdclass/0, add_if_no_exists_attrib/2, register_loader/2,
         register_loader/3, unregister_loader/2]).

-record(class_state, {classes = dict:new(), loaders = [] :: [callable()]}).
...
-spec get(ephp:context_id(),
          ephp_ns:namespace(),
          class_name(),
          AutoLoad :: boolean() | spl) ->
             get_return().
        ...
        true ->
            case erlang:get(Classes) of
                ...
                #class_state{loaders = RLoaders} ->
                    Loaders =
                        case lists:member(<<"__autoload">>, RLoaders) of
                            true ->
                                RLoaders;
                            false ->
                                RLoaders ++ [<<"__autoload">>]
...
-spec set_alias(ephp:classes_id(),
                ephp_ns:namespace(),
                class_name(),
                NSAlias :: ephp_ns:namespace(),
                Alias :: class_name()) ->
                   alias_return().
...
-spec register_class(ephp:classes_id(), File :: binary(), ephp:context_id(), class()) ->
                        ok.
...
register_class(Ref,
    ...
    case check_dup(PHPClass#class.implements) of
        ...
        {DupInterfaceNS, DupInterfaceName} ->
            ...
            ephp_error:error({error, edupinterface, Index, ?E_ERROR, {Name, DupInterface}})
    end,
    ...
    case check_final_methods(Ref,
                             PHPClass#class.methods,
    ...
    of
        ok ->
            ok;
        {error, {Class, Method}} ->
            ephp_error:error({error, efinalmethod, Index, ?E_ERROR, {Class, Method}})
    end,
    ...
    ActivePHPClass =
        PHPClass#class{static_context = Ctx,
                       parents = Parents,
                       file = File,
                       methods =
                           [CM#class_method{class_name = Name, namespace = NS}
                            || CM <- ClassMethods]
...
check_access_level([#class_attr{access = Access, name = Name} = CA1 | Rest]) ->
    case lists:keyfind(Name, #class_attr.name, Rest) of
        ...
        #class_attr{access = OtherAccess} = CA2 ->
            {error, {CA1#class_attr.class_name, Name, OtherAccess, CA2#class_attr.class_name}};
        false ->
            check_access_level(Rest)
    end.
...
-spec attrs_set_class_name(Name :: binary(), [class_attr()]) -> [class_attr()].
...
-spec extract_parents(ephp:classes_id(),
                      ephp_ns:namespace(),
...
extract_methods(Ref, Index, Implements) when is_reference(Ref) ->
    AllMethodsDict =
        lists:foldl(fun({NS, I}, D) ->
                    ...
                    [],
                    lists:reverse(Implements)),
    [V || {_, V} <- AllMethodsDict].

-type methods_dict() :: [{MethodName :: binary(), {class_name(), class_method()}}].

-spec extract_methods(class_name(), Index :: mixed(), [class_name()], methods_dict()) ->
                         methods_dict().
%% @hidden
...
extract_methods(Name, Index, [Method | Methods], MethodsDict) ->
    ...
    case lists:keyfind(MethodName, 1, MethodsDict) of
        false ->
            extract_methods(Name, Index, Methods, [{MethodName, {Name, Method}} | MethodsDict]);
...
%% @hidden
arg_to_text(#variable{name = Name}) ->
    <<"$", Name/binary>>.

-spec check_dup([{ephp_ns:namespace(), class_name()}]) ->
                   ok | {ephp_ns:namespace(), class_name()}.
%% @hidden
check_dup([]) ->
    ok;
check_dup([_Interface]) ->
    ok;
...
-spec check_methods([{class_name(), class_method()}],
                    [class_method()],
                    [{class_name(), class_method()}]) ->
                       [{class_name(), class_method()}].
%% @hidden
...
check_methods([{ClassName, Method} | Methods], ClassMethods, Error) ->
    Res = lists:any(fun(#class_method{name = Name, args = Args}) ->
                       (Name =:= Method#class_method.name)
                       and (length(Args) =:= length(Method#class_method.args))
                    end,
                    ClassMethods),
    NewError =
        case Res of
...
-type check_final_methods_return() :: ok | {error, {class_name(), method_name()}}.

-spec check_final_methods(ephp:classes_id(),
                          [class_method()],
                          ephp_ns:namespace(),
                          class_name()) ->
...
check_final_methods(Ref, Methods, ParentNS, ParentClass) ->
    {ok, #class{methods = ParentMethods, extends = Extends}} =
        get(Ref, ParentNS, ParentClass),
    case check_final_methods(Methods, ParentMethods) of
        ok ->
            check_final_methods(Ref, Methods, ParentNS, Extends);
        {error, MethodError} ->
            {error, {ParentClass, MethodError}}
    end.
...
get_extends_consts(Ref,
    ...
    case get(Ref, ExtendsNS, Extends, true) of
        ...
        {ok, #class{}} ->
            ephp_error:error({error, ecannotimpl, Index, ?E_ERROR, {Name, Extends}});
        _ ->
...
get_extends_consts(Ref,
    ...
    case get(Ref, ExtendsNS, Extends, true) of
        {ok, #class{name = FName, final = true}} ->
            ephp_error:error({error, efinalclass, Index, ?E_ERROR, {FName, Name}});
        {ok, #class{type = interface}} ->
            ephp_error:error({error, ecannotextends, Index, ?E_ERROR, {Name, Extends}});
        {ok, #class{} = Class} ->
...
instance(_Ref, LocalCtx, GlobalCtx, ClassNS, ClassName, Line) ->
    case get(LocalCtx, ClassNS, ClassName, true) of
        ...
        {error, enoexist} ->
            ephp_error:error({error, eundefclass, Line, ?E_ERROR, {ClassNS, ClassName}})
    end.

initialize_class(#class{static_context = Ctx, attrs = Attrs}) ->
    Bulk =
        lists:foldl(fun (#class_attr{type = static,
                                     name = Name,
                                     init_value = RawVal},
                         Acc) ->
...
initialize(Ctx, _ClassName, #class{attrs = Attrs}) ->
    Bulk =
        lists:foldl(fun (#class_attr{type = normal,
                                     access = private,
                                     class_name = CName,
                                     namespace = NS,
                                     name = Name,
        ...
        #class_method{static = Static} = Method ->
            case orddict:find(VarName, Static) of
                ...
                error ->
                    ...
                    NewMethods =
                        lists:keyreplace(MethodName, #class_method.name, Methods, NewMethod),
        ...
        #class_method{static = Static} = Method ->
            NewStatic =
                lists:map(fun({Key, _}) ->
                          ...
                          Static),
            NewMethod = Method#class_method{static = NewStatic},
            NewMethods = lists:keyreplace(MethodName, #class_method.name, Methods, NewMethod),
            NewClass = Class#class{methods = NewMethods},
            set(Ref, NS, ClassName, NewClass);
...
add_if_no_exists_attrib(#class{} = Class, Names) when is_list(Names) ->
    lists:foldl(fun(Name, C) -> add_if_no_exists_attrib(C, Name) end, Class, Names);
...
register_classes(ClassesRef, Context, Classes) ->
    ...
    lists:foreach(fun (#class{type = normal} = Class) ->
...
get_unsafe_extends_consts(_Classes, #class{extends = undefined}) ->
    [];
get_unsafe_extends_consts(Classes,
                          #class{extends = Extends,
                                 extends_ns = ExtendsNS,
                                 type = interface}) ->
...
get_unsafe_extends_consts(Classes, #class{extends = Extends, extends_ns = ExtendsNS}) ->
    {ok, #class{} = Class} = get(Classes, ExtendsNS, Extends),
    get_consts(Class) ++ get_unsafe_extends_consts(Classes, Class).

register_unsafe_class(Classes,
                      Consts,
                      GlobalCtx,
                      #class{name = Name,
                             constants = ConstDef0,
...
register_unsafe_interface(Classes,
    ...
    ActivePHPInterface = PHPInterface#class{file = <<"built-in">>, constants = ConstDef},
...
get_traversable() ->
    #class{name = <<"Traversable">>, type = interface}.

get_iterator_aggregate() ->
    M = #class_method{type = abstract},
    #class{name = <<"IteratorAggregate">>,
           type = interface,
           extends = <<"Traversable">>,
           methods = [M#class_method{name = <<"getIterator">>}]}.
...
get_array_access() ->
    ...
    MV = M#class_method{args =
                            [#variable{name = <<"offset">>}, #variable{name = <<"value">>}]},
