...
-export([start_link/0, destroy/1, set_output/2, set_output_handler/2,
         add_message_handler/2, set_exception_handler_func/2, get_exception_handler_func/1,
         remove_exception_handler_func/1, set_error_handler_func/3, get_error_handler_func/1,
         remove_error_handler_func/1, get_last/1, clear_last/1, run_quiet/2, error_reporting/2,
         error/1, handle_error/2, get_line/1, get_level/1]).
...
get_exception_handler_func(Context) ->
    ErrorState =
        erlang:get(
            ephp_context:get_errors_id(Context)),
...
-type throw_error() :: atom() | {error, error_type(), line(), error_level(), any()}.

-spec error(throw_error()) -> ok.
error({error, Type, Index, Level, Data}) ->
    throw({error, Type, Index, Level, Data}).

-spec handle_error(ephp:context_id(),
                   {error, error_type(), line(), binary(), error_level(), any()}) ->
                      get_return_return().
handle_error(Context, {error, euncaught, Index, File, Level, {File, Line, Exception}}) ->
    ErrorsId = ephp_context:get_errors_id(Context),
    case erlang:get(ErrorsId) of
        ...
        #state{exception_handler = ExceptionHandler} ->
            Call =
                #call{name = ExceptionHandler,
                      args = [Exception],
                      line = Index},
            ephp_context:solve(Context, Call)
    end,
    get_return(euncaught);
handle_error(Context, {error, _Type, Index, File, Level, _Data})
    when Level =:= 0 orelse Level band bnot ?E_ALL =/= 0 ->
...
handle_error(Context, {error, Type, Index, File, Level, Data}) ->
    ...
    Format = get_format(),
    {SimpleErrText, ErrorText} = get_message(Modules, Format, Type, Line, File, Level, Data),
    if Type =/= euncaught ->
           LastError =
               ephp_array:from_list([{<<"type">>, Level},
    ...
    case ErrorState of
        #state{error_handler = [{ErrHandler, CfgLevel} | _], output_handler = Module}
            when CfgLevel band Level =/= 0 andalso ?E_HANDLE_ERRORS band Level =/= 0 ->
            case run(Context, ErrorState, ErrHandler, Level, SimpleErrText, File, Index) of
                false ->
                    Module:set_output(Context, iolist_to_binary(ErrorText)),
                    if ?E_EXIT_ON_FALSE band Level =/= 0 ->
                           throw({ok, die});
                       true ->
                           ok
                    end;
...
run(Context, State, ErrHandler, Level, ErrorText, File, Index) ->
    ...
    Args =
        [#int{int = Level},
         #text{text = iolist_to_binary(ErrorText)},
         #text{text = File},
         #int{int = get_line(Index)}],
    Call =
        #call{name = ErrHandler,
              args = Args,
              line = Index},
    Return =
...
get_message(Modules, text, Type, Line, File, Level, Args) ->
    ...
    {Message,
     io_lib:format("~n~s: ~s in ~s on line ~p~n", [get_level(Level), Message, File, Line])};
get_message(Modules, html, Type, Line, File, Level, Args) ->
    Message = get_message(Modules, Type, Level, Args),
    {Message,
     io_lib:format("<br/>~n<b>~s</b>: ~s in <b>~s</b> on line <b>~p</b><br/>~n",
...
get_message([Module | Modules], Type, Level, Args) ->
    case Module:handle_error(Type, Level, Args) of
        Message when is_list(Message) ->
            Message;
        ignore ->
            get_message(Modules, Type, Level, Args)
    end.
...
get_message(eparse, {Value, Type, Expected}) when is_binary(Type) ->
    io_lib:format("syntax error, unexpected '~s' (~s), expecting '~s'",
                  [Value, Type, Expected]);
...
get_message(ewrongminarity, {Function, NumArgsExp, NumArgsSent}) ->
    io_lib:format("~s() expects at least ~p parameters, ~p given",
                  [Function, NumArgsExp, NumArgsSent]);
get_message(ewrongmaxarity, {Function, NumArgsExp, NumArgsSent}) ->
    io_lib:format("~s() expects at most ~p parameters, ~p given",
                  [Function, NumArgsExp, NumArgsSent]);
get_message(ewrongarg, {Function, ArgNum, ArgType, WrongType}) ->
    io_lib:format("~s() expects parameter ~p to be ~s, ~s given",
...
get_message(eundefconst, {_NS, Const}) ->
    io_lib:format("Use of undefined constant ~s - assumed '~s'", [Const, Const]);
get_message(enotostring, {ClassName}) ->
    io_lib:format("Object of class ~s could not be converted to string", [ClassName]);
get_message(enocast, {ClassName, Type}) ->
    io_lib:format("Object of class ~s could not be converted to ~s", [ClassName, Type]);
...
get_message(euncaught, {File, Line, Exception, Ctx}) ->
    ...
    Traces =
        lists:map(fun({K, V}) -> trace_to_str(Ctx, K, V) end, ephp_array:to_list(StackTrace)),
    ...
    io_lib:format("Uncaught exception '~s'"
                  ++ MessageFmt
...
get_message(errtype, {I, ReqType, GivenType, FuncName}) ->
    io_lib:format("Argument ~p passed to ~s() must be an instance of ~s, "
                  "~s given, called",
                  [I, FuncName, ReqType, GivenType]);
get_message(einvalidcallback, {Func, Arg}) ->
    io_lib:format("~s() expects the argument (~s) to be a valid callback", [Func, Arg]);
...
trace_to_str(Ctx, I, Array) ->
    ...
    ArgStrList =
        lists:map(fun ({_, #var_ref{pid = Vars, ref = Var}}) ->
                          binary_to_list(ephp_string:escape(
                                             ephp_vars:get(Vars, Var, Ctx), $'));
                  ...
                  ephp_array:to_list(RawArgList)),
    Args = string:join(ArgStrList, ","),
    case File of
        undefined ->
            io_lib:format("#~p [internal function]: ~s(~s)~n", [I, FuncName, Args]);
        _ ->
            io_lib:format("#~p ~s(~p): ~s(~s)~n", [I, File, Line, FuncName, Args])
    end.
...
get_level(?E_NOTICE) ->
    <<"Notice">>;
get_level(?E_CORE_ERROR) ->
    <<"Error">>;
get_level(?E_CORE_WARNING) ->
    <<"Warning">>;
get_level(?E_COMPILE_ERROR) ->
    <<"Error">>;
get_level(?E_COMPILE_WARNING) ->
    <<"Warning">>;
get_level(?E_USER_ERROR) ->
    <<"Fatal error">>;
get_level(?E_USER_WARNING) ->
    <<"Warning">>;
get_level(?E_USER_NOTICE) ->
    <<"Notice">>;
get_level(?E_STRICT) ->
    <<"Strict Standards">>;
get_level(?E_RECOVERABLE_ERROR) ->
    <<"Catchable fatal error">>;
get_level(?E_DEPRECATED) ->
    <<"Deprecated">>;
get_level(?E_USER_DEPRECATED) ->
    <<"Deprecated">>;
get_level(_) ->
    <<"Unknown">>.
