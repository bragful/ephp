...
init_func() -> [
    ...
    str_split,
    {strpos, [
        {args, {2, 3, undefined, [string,
                                  {str_or_int, undefined,
                                   {eisnot, [<<"needle">>,
    ...
    {md5, [
        {args, {1, 2, undefined, [string, {boolean, false}]}}
    ]},
    {bin2hex, [
        {args, {1, 1, undefined, [string]}}
    ]},
...
strrpos(Context, Line, {_, HayStack}, {_, Needle},
...
md5(_Context, _Line, {_, Binary}, {_, Raw}) ->
    case ephp_data:to_boolean(Raw) of
        true -> erlang:md5(Binary);
        false -> ephp_string:bin2hex(erlang:md5(Binary))
    end.
