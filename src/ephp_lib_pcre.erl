...
-export([init_func/0, init_config/0, init_const/0, preg_match/7, preg_replace/7,
         preg_quote/4]).
...
init_func() ->
    [{preg_match,
      [{args,
        {2, 5, false, [string, string, {raw, ephp_array:new()}, {integer, 0}, {integer, 0}]}}]},
     {preg_replace,
      [{args, {3, 5, undefined, [mixed, mixed, mixed, {integer, -1}, {integer, 0}]}}]},
     {preg_quote, [{args, {1, 2, undefined, [string, {string, <<>>}]}}]}].

-spec init_config() -> ephp_lib:php_config_results().
init_config() ->
    [{<<"pcre.backtrack_limit">>, 1000000}, {<<"pcre.recurssion_limit">>, 100000}].
...
-spec preg_replace(ephp:context_id(),
                   line(),
                   Pattern :: var_value(),
        ...
        case re:run(Subject, RegExp, Flags ++ PMFlags) of
            {match, Matches} ->
                if Flag =:= ?PREG_OFFSET_CAPTURE ->
                       ...
                       Elements =
                           lists:zipwith(fun({I, _}, Chunk) -> ephp_array:from_list([Chunk, I]) end,
                                         Idx,
                                         Matches),
...
escape(<<A/utf8, Str/binary>>, StrRes, Delims) ->
    case lists:member(A, Delims) of
        true ->
            escape(Str, <<StrRes/binary, "\\", A/utf8>>, Delims);
        false ->
            escape(Str, <<StrRes/binary, A/utf8>>, Delims)
    end.
...
get_end_delimiter(${) ->
    $};
get_end_delimiter($\\) ->
    throw({error, edelimiter});
get_end_delimiter(A) when ?IS_ALPHA(A) ->
    throw({error, edelimiter});
get_end_delimiter(A) when ?IS_NUMBER(A) ->
    throw({error, edelimiter});
get_end_delimiter(A) when ?IS_SPACE(A) ->
    throw({error, edelimiter});
get_end_delimiter(A) ->
    A.
