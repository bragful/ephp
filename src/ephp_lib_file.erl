...
-export([init_func/0, init_config/0, init_const/0, handle_error/3, basename/3, dirname/3,
         file_exists/3, is_dir/3, is_readable/3, fopen/4, fclose/3, file_get_contents/3, fread/4,
         fwrite/5, fseek/5, feof/3, glob/4]).
     ...
     {file_get_contents,
      [{args,
        {1,
         5,
         false,
         [string, {boolean, false}, {resource, undefined}, {integer, 0}, {integer, undefined}]}},
       {pack_args, true}]},
     {fread, [{args, {2, 2, false, [resource, integer]}}]},
     {fwrite, [{args, {2, 3, false, [resource, string, {integer, 0}]}}]},
     {fseek, [{args, {2, 3, -1, [resource, integer, {integer, ?SEEK_SET}]}}]},
...
handle_error(einval, _Level, {Function, DescriptorNum}) ->
    io_lib:format("~s(): ~p is not a valid stream resource", [Function, DescriptorNum]);
...
-spec basename(ephp:context_id(), line(), var_value()) -> binary().
basename(_Context, _Line, {_Var, undefined}) ->
    <<>>;
basename(_Context, _Line, {_Var, PathFile}) ->
    filename:basename(
        ephp_data:to_bin(PathFile)).

-spec dirname(ephp:context_id(), line(), var_value()) -> binary().
...
-spec file_exists(ephp:context_id(), line(), var_value()) -> boolean().
file_exists(_Context, _Line, {_, Filename}) ->
    ephp_stream:file_exists(
        ephp_data:to_bin(Filename)).

-spec is_dir(ephp:context_id(), line(), var_value()) -> boolean().
is_dir(_Context, _Line, {_, Dirname}) ->
    ephp_stream:is_dir(
        ephp_data:to_bin(Dirname)).

-spec is_readable(ephp:context_id(), line(), var_value()) -> boolean().
is_readable(_Context, _Line, {_, Filename}) ->
    ephp_stream:is_readable(
        ephp_data:to_bin(Filename)).

-spec fopen(ephp:context_id(), line(), var_value(), var_value()) -> resource() | false.
fopen(Context, Line, {_, Filename}, {_, Mode}) ->
    ...
    case ephp_stream:open(Filename, Opts) of
        ...
        {error, enoent} ->
            ...
            ephp_error:handle_error(Context, {error, enoent, Line, File, ?E_WARNING, Data}),
            false
    end.

-spec fclose(ephp:context_id(), line(), var_value()) -> boolean().
fclose(Context, Line, {_, Resource}) ->
    case ephp_stream:close(Resource) of
        ...
        {error, einval} ->
            ...
            File = ephp_context:get_active_file(Context),
            ephp_error:handle_error(Context, {error, einval, Line, File, ?E_WARNING, Data}),
            false
    end.

-spec file_get_contents(ephp:context_id(), line(), [var_value()]) -> false | binary().
file_get_contents(_Context,
                  _Line,
                  [{_, File}, {_, false}, {_, undefined}, {_, 0}, {_, undefined}]) ->
...
-spec fread(ephp:context_id(), line(), var_value(), var_value()) -> binary().
...
-spec fwrite(ephp:context_id(), line(), var_value(), var_value(), var_value()) ->
...
fwrite(Context, Line, {_, Resource}, {_, Data}, {_, Length}) ->
    ...
    case ephp_stream:write(Resource, WriteData, []) of
        ...
        {error, einval} ->
            ...
            File = ephp_context:get_active_file(Context),
            ephp_error:handle_error(Context, {error, einval, Line, File, ?E_WARNING, ErrData}),
            false
    end.

-spec fseek(ephp:context_id(),
            line(),
            Handle :: var_value(),
            Offset :: var_value(),
            Whence :: var_value()) ->
