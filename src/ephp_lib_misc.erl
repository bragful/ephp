...
-export([init_func/0, init_config/0, init_const/0, handle_error/3, eval/3, constant/3,
         uniqid/4, define/4, defined/3, sleep/3, usleep/3, exit/3, pack/3, unpack/4]).
...
init_func() ->
    [{eval, [string]},
     {constant, [string]},
     {uniqid, [{args, {0, 2, undefined, [{string, <<>>}, {boolean, false}]}}]},
     {define, [string, mixed]},
     {defined, [string]},
     {sleep, [integer]},
     {usleep, [integer]},
     {exit, [{args, {0, 1, undefined, [{mixed, undefined}]}}, {alias, <<"die">>}]},
     {exit, [{args, {0, 1, undefined, [{mixed, undefined}]}}]},
     {pack, [pack_args]},
     {unpack, [{args, {2, 2, undefined, [string, string]}}]}].
...
eval(Context, _Line, {_, Code}) ->
    ...
    case ephp:eval(File, SubCtx, <<"<?php ", Code/binary>>) of
        ...
        {ok, {return, Value}} ->
            Value;
        {error, eparse, _ErrorLine, _File, _ErrorLevel, _Data} ->
            undefined
    end.
...
uniqid(_Context, _Line, {_, Prefix}, {_, Extended}) ->
    ...
    Rand =
        case ephp_data:to_bool(Extended) of
            true ->
                float_to_binary(ephp_data:urand(), [compact, {decimals, 8}]);
            false ->
                <<>>
        end,
    <<Prefix/binary, Str/binary, Rand/binary>>.

-spec define(ephp:context_id(),
             line(),
             Constant :: var_value(),
             Content :: var_value()) ->
                true.
define(Context, _Line, {#text{}, Constant}, {_, Content}) ->
    ...
    true;
define(Context, _Line, {#constant{name = Constant}, _}, {_UnParsedContent, Content}) ->
    ephp_context:register_const(Context, Constant, Content),
    true.
...
-spec usleep(ephp:context_id(), line(), MicroSeconds :: var_value()) -> false | integer().
...
-spec exit(ephp:context_id(), line(), Message :: var_value()) -> undefined.
exit(_Context, _Line, {_, Value}) when is_integer(Value) orelse Value =:= undefined ->
    throw(die);
...
pack(Context, Line, [{_, Format} | Args]) ->
    do_pack(Context, Line, ephp_data:to_bin(Format), [A || {_, A} <- Args], <<>>).

-spec unpack(ephp:context_id(), line(), var_value(), var_value()) ->
                ephp_array:ephp_array().
unpack(Context, Line, {_, Format}, {_, Binary}) ->
    do_unpack(Context,
              Line,
              binary:split(
                  ephp_data:to_bin(Format), <<"/">>, [global]),
              Binary,
              ephp_array:new(),
              <<>>).
...
do_unpack(_Context, _Line, [], _Binary, Array, _PrevBin) ->
    Array;
do_unpack(Context, Line, [<<"a*", Rest/binary>> | Format], Binary, Array, PrevBin) ->
    ...
    NewArray =
        case Rest of
            ...
            _ ->
                ephp_array:store(Rest, String, Array)
        end,
...
do_unpack(Context, Line, [<<"a", Rest/binary>> | Format], Binary, Array, PrevBin) ->
    ...
    case Size - byte_size(String) of
        N when N > 0 ->
            ...
            ephp_error:handle_error(Context, {error, enoenoughin, Line, File, ?E_WARNING, Data}),
            false;
        N when N =:= 0 ->
...
do_unpack(Context, Line, [<<"Z*", Rest/binary>> | Format], Binary, Array, PrevBin) ->
    String =
        ephp_string:rtrim(
            ephp_data:to_bin(Binary), [0]),
    NewArray =
        case Rest of
            <<>> ->
                ephp_array:store(1, String, Array);
...
do_unpack(Context, Line, [<<"h*", Rest/binary>> | Format], Binary, Array, PrevBin) ->
    String =
        ephp_string:ibin2hex(
            ephp_data:to_bin(Binary)),
    NewArray =
        case Rest of
            <<>> ->
                ephp_array:store(1, String, Array);
...
do_unpack(Context, Line, [<<"c*", Idx/binary>> | Format], Binary, Array, PrevBin) ->
    %% FIXME when R16 is not supported anymore it could be rewrote:
    Process =
        fun (_P, _I, Array0, <<>>, Prev) ->
                {Array0, Prev};
            (P, I, Array0, <<Char:8/integer-signed, String/binary>>, Prev) when Idx =:= <<>> ->
...
do_unpack(Context, Line, [<<"c", Rest/binary>> | Format], Binary, Array, PrevBin) ->
    {Size, Idx} = get_numbers(Rest, 1),
    Process =
        fun (_P, _I, Array0, <<>>, 0, Prev) ->
                do_unpack(Context, Line, Format, <<>>, Array0, Prev);
            (_P, _I, _Array0, <<>>, _N, _Prev) ->
                File = ephp_context:get_active_file(Context),
                Data = {<<"unpack">>, <<"c">>, 1, 0},
                ephp_error:handle_error(Context,
                                        {error, enoenoughin, Line, File, ?E_WARNING, Data}),
                false;
            (_P, 1, Array0, <<Char:8/integer-signed, String/binary>>, 1, Prev) when Idx =/= <<>> ->
            ...
            (P, I, Array0, <<Char:8/integer-signed, String/binary>>, N, Prev) when Idx =:= <<>> ->
                Array1 = ephp_array:store(I, Char, Array0),
                PrevStr = <<Prev/binary, Char:8/integer-signed>>,
...
do_unpack(Context, Line, [<<"C*", Idx/binary>> | Format], Binary, Array, PrevBin) ->
    ...
    Process =
        fun (_P, _I, Array0, <<>>, Prev) ->
                do_unpack(Context, Line, Format, <<>>, Array0, Prev);
            (P, I, Array0, <<Char:8/integer-unsigned, String/binary>>, Prev) when Idx =:= <<>> ->
                Array1 = ephp_array:store(I, Char, Array0),
...
do_unpack(Context, Line, [<<"C", Rest/binary>> | Format], Binary, Array, PrevBin) ->
    ...
    Process =
        fun (_P, _I, Array0, <<>>, 0, Prev) ->
            ...
            (_P, _I, Array0, String, 0, Prev) ->
                do_unpack(Context, Line, Format, String, Array0, Prev);
            (P, I, Array0, <<Char:8/integer-unsigned, String/binary>>, N, Prev) when Idx =:= <<>> ->
                Array1 = ephp_array:store(I, Char, Array0),
                PrevStr = <<Prev/binary, Char:8/integer-unsigned>>,
                P(P, I + 1, Array1, String, N - 1, PrevStr);
    ...
    Process(Process, 1, Array, Binary, Size, PrevBin);
do_unpack(Context, Line, [<<"s*", Idx/binary>> | Format], Binary, Array, PrevBin) ->
    %% FIXME when R16 is not supported anymore it could be rewrote:
    Process =
        fun (_P, _I, Array0, String, Prev) when bit_size(String) < 16 ->
                {Array0, Prev};
    ...
    {Array0, PrevBin0} = Process(Process, 1, Array, Binary, PrevBin),
    do_unpack(Context, Line, Format, <<>>, Array0, PrevBin0);
do_unpack(Context, Line, [<<"s", Rest/binary>> | Format], Binary, Array, PrevBin) ->
    {Size, Idx} = get_numbers(Rest, 1),
    Process =
        fun (_P, _I, Array0, String, 0, Prev) when bit_size(String) < 16 ->
                do_unpack(Context, Line, Format, <<>>, Array0, Prev);
            (_P, _I, _Array0, String, _N, _Prev) when bit_size(String) < 16 ->
                ...
                ephp_error:handle_error(Context,
                                        {error, enoenoughin, Line, File, ?E_WARNING, Data}),
                false;
            (_P, 1, Array0, <<Bin:16/integer-signed-little, String/binary>>, 1, Prev)
                when Idx =/= <<>> ->
    ...
    Process(Process, 1, Array, Binary, Size, PrevBin);
do_unpack(Context, Line, [<<"n*", Idx/binary>> | Format], Binary, Array, PrevBin) ->
    %% FIXME when R16 is not supported anymore it could be rewrote:
    Process =
        fun (_P, _I, Array0, String, Prev) when bit_size(String) < 16 ->
                {Array0, Prev};
    ...
    {Array0, PrevBin0} = Process(Process, 1, Array, Binary, PrevBin),
    do_unpack(Context, Line, Format, <<>>, Array0, PrevBin0);
do_unpack(Context, Line, [<<"n", Rest/binary>> | Format], Binary, Array, PrevBin) ->
    {Size, Idx} = get_numbers(Rest, 1),
    Process =
        fun (_P, _I, Array0, String, 0, Prev) when bit_size(String) < 16 ->
                do_unpack(Context, Line, Format, <<>>, Array0, Prev);
            (_P, _I, _Array0, String, _N, _Prev) when bit_size(String) < 16 ->
                ...
                ephp_error:handle_error(Context,
                                        {error, enoenoughin, Line, File, ?E_WARNING, Data}),
                false;
            (_P, 1, Array0, <<Bin:16/integer-unsigned-big, String/binary>>, 1, Prev)
                when Idx =/= <<>> ->
                Array1 = ephp_array:store(Idx, Bin, Array0),
                PrevStr = <<Prev/binary, Bin:16/integer-unsigned-big>>,
                do_unpack(Context, Line, Format, String, Array1, PrevStr);
    ...
    Process(Process, 1, Array, Binary, Size, PrevBin);
do_unpack(Context, Line, [<<A:8, "*", Idx/binary>> | Format], Binary, Array, PrevBin)
    when A =:= $v orelse A =:= $S ->
    %% FIXME when R16 is not supported anymore it could be rewrote:
    Process =
        fun (_P, _I, Array0, String, Prev) when bit_size(String) < 16 ->
    ...
    {Array0, PrevBin0} = Process(Process, 1, Array, Binary, PrevBin),
    do_unpack(Context, Line, Format, <<>>, Array0, PrevBin0);
do_unpack(Context, Line, [<<A:8, Rest/binary>> | Format], Binary, Array, PrevBin)
    when A =:= $v orelse A =:= $S ->
    {Size, Idx} = get_numbers(Rest, 1),
    Process =
        fun (_P, _I, Array0, String, 0, Prev) when bit_size(String) < 16 ->
            ...
            (_P, _I, _Array0, String, _N, _Prev) when bit_size(String) < 16 ->
                ...
                ephp_error:handle_error(Context,
                                        {error, enoenoughin, Line, File, ?E_WARNING, Data}),
                false;
            (_P, 1, Array0, <<Bin:16/integer-unsigned-little, String/binary>>, 1, Prev)
                when Idx =/= <<>> ->
                Array1 = ephp_array:store(Idx, Bin, Array0),
                PrevStr = <<Prev/binary, Bin:16/integer-unsigned-little>>,
                do_unpack(Context, Line, Format, String, Array1, PrevStr);
            (_P, _I, Array0, String, 0, Prev) ->
                do_unpack(Context, Line, Format, String, Array0, Prev);
            (P, I, Array0, <<Bin:16/integer-unsigned-little, String/binary>>, N, Prev)
                when Idx =:= <<>> ->
                Array1 = ephp_array:store(I, Bin, Array0),
                PrevStr = <<Prev/binary, Bin:16/integer-unsigned-little>>,
                P(P, I + 1, Array1, String, N - 1, PrevStr);
    ...
    Process(Process, 1, Array, Binary, Size, PrevBin);
do_unpack(Context, Line, [<<A:8, "*", Idx/binary>> | Format], Binary, Array, PrevBin)
    when A =:= $I orelse A =:= $L orelse A =:= $V ->
    %% FIXME when R16 is not supported anymore it could be rewrote:
    Process =
        fun (_P, _I, Array0, String, Prev) when bit_size(String) < 32 ->
                {Array0, Prev};
...
do_unpack(Context, Line, [<<A:8, Rest/binary>> | Format], Binary, Array, PrevBin)
    ...
    {Size, Idx} = get_numbers(Rest, 1),
    Process =
        fun (_P, _I, Array0, String, 0, Prev) when bit_size(String) < 32 ->
                do_unpack(Context, Line, Format, <<>>, Array0, Prev);
            (_P, _I, _Array0, String, _N, _Prev) when bit_size(String) < 32 ->
                ...
                ephp_error:handle_error(Context,
                                        {error, enoenoughin, Line, File, ?E_WARNING, Data}),
                false;
            (_P, 1, Array0, <<Bin:32/integer-unsigned-little, String/binary>>, 1, Prev)
                when Idx =/= <<>> ->
                Array1 = ephp_array:store(Idx, Bin, Array0),
                PrevStr = <<Prev/binary, Bin:32/integer-unsigned-little>>,
                do_unpack(Context, Line, Format, String, Array1, PrevStr);
            (_P, _I, Array0, String, 0, Prev) ->
                do_unpack(Context, Line, Format, String, Array0, Prev);
            (P, I, Array0, <<Bin:32/integer-unsigned-little, String/binary>>, N, Prev)
                when Idx =:= <<>> ->
                Array1 = ephp_array:store(I, Bin, Array0),
                PrevStr = <<Prev/binary, Bin:32/integer-unsigned-little>>,
                P(P, I + 1, Array1, String, N - 1, PrevStr);
        ...
        fun (_P, _I, Array0, String, 0, Prev) when bit_size(String) < 32 ->
            ...
            (_P, _I, _Array0, String, _N, _Prev) when bit_size(String) < 32 ->
                ...
                ephp_error:handle_error(Context,
                                        {error, enoenoughin, Line, File, ?E_WARNING, Data}),
                false;
            (_P, 1, Array0, <<Bin:32/integer-signed-little, String/binary>>, 1, Prev)
                when Idx =/= <<>> ->
                Array1 = ephp_array:store(Idx, Bin, Array0),
                PrevStr = <<Prev/binary, Bin:32/integer-signed-little>>,
                do_unpack(Context, Line, Format, String, Array1, PrevStr);
            (_P, _I, Array0, String, 0, Prev) ->
                do_unpack(Context, Line, Format, String, Array0, Prev);
            (P, I, Array0, <<Bin:32/integer-signed-little, String/binary>>, N, Prev)
                when Idx =:= <<>> ->
                Array1 = ephp_array:store(I, Bin, Array0),
                PrevStr = <<Prev/binary, Bin:32/integer-signed-little>>,
                P(P, I + 1, Array1, String, N - 1, PrevStr);
        ...
        fun (_P, _I, Array0, String, 0, Prev) when bit_size(String) < 32 ->
            ...
            (_P, _I, _Array0, String, _N, _Prev) when bit_size(String) < 32 ->
                ...
                ephp_error:handle_error(Context,
                                        {error, enoenoughin, Line, File, ?E_WARNING, Data}),
                false;
            (_P, 1, Array0, <<Bin:32/integer-unsigned-big, String/binary>>, 1, Prev)
                when Idx =/= <<>> ->
                Array1 = ephp_array:store(Idx, Bin, Array0),
                PrevStr = <<Prev/binary, Bin:32/integer-unsigned-big>>,
                do_unpack(Context, Line, Format, String, Array1, PrevStr);
            (_P, _I, Array0, String, 0, Prev) ->
                do_unpack(Context, Line, Format, String, Array0, Prev);
            (P, I, Array0, <<Bin:32/integer-unsigned-big, String/binary>>, N, Prev)
                when Idx =:= <<>> ->
                Array1 = ephp_array:store(I, Bin, Array0),
                PrevStr = <<Prev/binary, Bin:32/integer-unsigned-big>>,
                P(P, I + 1, Array1, String, N - 1, PrevStr);
...
do_unpack(Context, Line, [<<A:8, "*", Idx/binary>> | Format], Binary, Array, PrevBin)
    ...
    %% FIXME when R16 is not supported anymore it could be rewrote:
    Process =
        fun (_P, _I, Array0, String, Prev) when bit_size(String) < 64 ->
                {Array0, Prev};
...
do_unpack(Context, Line, [<<A:8, Rest/binary>> | Format], Binary, Array, PrevBin)
    ...
    {Size, Idx} = get_numbers(Rest, 1),
    Process =
        fun (_P, _I, Array0, String, 0, Prev) when bit_size(String) < 64 ->
                do_unpack(Context, Line, Format, <<>>, Array0, Prev);
            (_P, _I, _Array0, String, _N, _Prev) when bit_size(String) < 64 ->
                ...
                ephp_error:handle_error(Context,
                                        {error, enoenoughin, Line, File, ?E_WARNING, Data}),
                false;
            (_P, 1, Array0, <<Bin:64/integer-unsigned-little, String/binary>>, 1, Prev)
                when Idx =/= <<>> ->
                Array1 = ephp_array:store(Idx, Bin, Array0),
                PrevStr = <<Prev/binary, Bin:64/integer-unsigned-little>>,
                do_unpack(Context, Line, Format, String, Array1, PrevStr);
            (_P, _I, Array0, String, 0, Prev) ->
                do_unpack(Context, Line, Format, String, Array0, Prev);
            (P, I, Array0, <<Bin:64/integer-unsigned-little, String/binary>>, N, Prev)
                when Idx =:= <<>> ->
                Array1 = ephp_array:store(I, Bin, Array0),
                PrevStr = <<Prev/binary, Bin:64/integer-unsigned-little>>,
                P(P, I + 1, Array1, String, N - 1, PrevStr);
    ...
    Process(Process, 1, Array, Binary, Size, PrevBin);
do_unpack(Context, Line, [<<"q*", Idx/binary>> | Format], Binary, Array, PrevBin) ->
    %% FIXME when R16 is not supported anymore it could be rewrote:
    Process =
        fun (_P, _I, Array0, String, Prev) when bit_size(String) < 64 ->
                {Array0, Prev};
    ...
    {Array0, PrevBin0} = Process(Process, 1, Array, Binary, PrevBin),
    do_unpack(Context, Line, Format, <<>>, Array0, PrevBin0);
do_unpack(Context, Line, [<<"q", Rest/binary>> | Format], Binary, Array, PrevBin) ->
    {Size, Idx} = get_numbers(Rest, 1),
    Process =
        fun (_P, _I, Array0, String, 0, Prev) when bit_size(String) < 64 ->
                do_unpack(Context, Line, Format, <<>>, Array0, Prev);
            (_P, _I, _Array0, String, _N, _Prev) when bit_size(String) < 64 ->
                ...
                ephp_error:handle_error(Context,
                                        {error, enoenoughin, Line, File, ?E_WARNING, Data}),
                false;
            (_P, 1, Array0, <<Bin:64/integer-signed-little, String/binary>>, 1, Prev)
                when Idx =/= <<>> ->
                Array1 = ephp_array:store(Idx, Bin, Array0),
                PrevStr = <<Prev/binary, Bin:64/integer-signed-little>>,
                do_unpack(Context, Line, Format, String, Array1, PrevStr);
            (_P, _I, Array0, String, 0, Prev) ->
                do_unpack(Context, Line, Format, String, Array0, Prev);
            (P, I, Array0, <<Bin:64/integer-signed-little, String/binary>>, N, Prev)
                when Idx =:= <<>> ->
                Array1 = ephp_array:store(I, Bin, Array0),
                PrevStr = <<Prev/binary, Bin:64/integer-signed-little>>,
                P(P, I + 1, Array1, String, N - 1, PrevStr);
    ...
    Process(Process, 1, Array, Binary, Size, PrevBin);
do_unpack(Context, Line, [<<"J*", Idx/binary>> | Format], Binary, Array, PrevBin) ->
    %% FIXME when R16 is not supported anymore it could be rewrote:
    Process =
        fun (_P, _I, Array0, String, Prev) when bit_size(String) < 32 ->
                {Array0, Prev};
    ...
    {Array0, PrevBin0} = Process(Process, 1, Array, Binary, PrevBin),
    do_unpack(Context, Line, Format, <<>>, Array0, PrevBin0);
do_unpack(Context, Line, [<<"J", Rest/binary>> | Format], Binary, Array, PrevBin) ->
    {Size, Idx} = get_numbers(Rest, 1),
    Process =
        fun (_P, _I, Array0, String, 0, Prev) when bit_size(String) < 64 ->
                do_unpack(Context, Line, Format, <<>>, Array0, Prev);
            (_P, _I, _Array0, String, _N, _Prev) when bit_size(String) < 64 ->
                ...
                ephp_error:handle_error(Context,
                                        {error, enoenoughin, Line, File, ?E_WARNING, Data}),
                false;
            (_P, 1, Array0, <<Bin:64/integer-unsigned-big, String/binary>>, 1, Prev)
                when Idx =/= <<>> ->
                Array1 = ephp_array:store(Idx, Bin, Array0),
                PrevStr = <<Prev/binary, Bin:64/integer-unsigned-big>>,
                do_unpack(Context, Line, Format, String, Array1, PrevStr);
            (_P, _I, Array0, String, 0, Prev) ->
                do_unpack(Context, Line, Format, String, Array0, Prev);
            (P, I, Array0, <<Bin:64/integer-unsigned-big, String/binary>>, N, Prev)
                when Idx =:= <<>> ->
                Array1 = ephp_array:store(I, Bin, Array0),
                PrevStr = <<Prev/binary, Bin:64/integer-unsigned-big>>,
                P(P, I + 1, Array1, String, N - 1, PrevStr);
    ...
    Process(Process, 1, Array, Binary, Size, PrevBin);
do_unpack(Context, Line, [<<"f*", Idx/binary>> | Format], Binary, Array, PrevBin) ->
    %% FIXME when R16 is not supported anymore it could be rewrot, PrevBine:
    Process =
        fun (_P, _I, Array0, String, Prev) when bit_size(String) < 32 ->
                {Array0, Prev};
            (P, I, Array0, <<Bin:32/float-little, String/binary>>, Prev) when Idx =:= <<>> ->
                Array1 = ephp_array:store(I, Bin, Array0),
    ...
    {Array0, PrevBin0} = Process(Process, 1, Array, Binary, PrevBin),
    do_unpack(Context, Line, Format, <<>>, Array0, PrevBin0);
do_unpack(Context, Line, [<<"f", Rest/binary>> | Format], Binary, Array, PrevBin) ->
    {Size, Idx} = get_numbers(Rest, 1),
    Process =
        fun (_P, _I, Array0, String, 0, Prev) when bit_size(String) < 32 ->
                do_unpack(Context, Line, Format, <<>>, Array0, Prev);
            (_P, _I, _Array0, String, _N, _Prev) when bit_size(String) < 32 ->
                ...
                ephp_error:handle_error(Context,
                                        {error, enoenoughin, Line, File, ?E_WARNING, Data}),
                false;
            (_P, 1, Array0, <<Bin:32/float-little, String/binary>>, 1, Prev) when Idx =/= <<>> ->
                Array1 = ephp_array:store(Idx, Bin, Array0),
                PrevStr = <<Prev/binary, Bin:32/float-little>>,
                do_unpack(Context, Line, Format, String, Array1, PrevStr);
            (_P, _I, Array0, String, 0, Prev) ->
                do_unpack(Context, Line, Format, String, Array0, Prev);
            (P, I, Array0, <<Bin:32/float-little, String/binary>>, N, Prev) when Idx =:= <<>> ->
                Array1 = ephp_array:store(I, Bin, Array0),
                PrevStr = <<Prev/binary, Bin:32/float-little>>,
                P(P, I + 1, Array1, String, N - 1, PrevStr);
    ...
    Process(Process, 1, Array, Binary, Size, PrevBin);
do_unpack(Context, Line, [<<"d*", Idx/binary>> | Format], Binary, Array, PrevBin) ->
    %% FIXME when R16 is not supported anymore it could be rewrote:
    Process =
        fun (_P, _I, Array0, String, Prev) when bit_size(String) < 64 ->
                {Array0, Prev};
            (P, I, Array0, <<Bin:64/float-little, String/binary>>, Prev) when Idx =:= <<>> ->
                Array1 = ephp_array:store(I, Bin, Array0),
    ...
    {Array0, PrevBin0} = Process(Process, 1, Array, Binary, PrevBin),
    do_unpack(Context, Line, Format, <<>>, Array0, PrevBin0);
do_unpack(Context, Line, [<<"d", Rest/binary>> | Format], Binary, Array, PrevBin) ->
    {Size, Idx} = get_numbers(Rest, 1),
    Process =
        fun (_P, _I, Array0, String, 0, Prev) when bit_size(String) < 64 ->
                do_unpack(Context, Line, Format, <<>>, Array0, Prev);
            (_P, _I, _Array0, String, _N, _Prev) when bit_size(String) < 64 ->
                ...
                ephp_error:handle_error(Context,
                                        {error, enoenoughin, Line, File, ?E_WARNING, Data}),
                false;
            (_P, 1, Array0, <<Bin:64/float-little, String/binary>>, 1, Prev) when Idx =/= <<>> ->
                Array1 = ephp_array:store(Idx, Bin, Array0),
                PrevStr = <<Prev/binary, Bin:64/float-little>>,
                do_unpack(Context, Line, Format, String, Array1, PrevStr);
            (_P, _I, Array0, String, 0, Prev) ->
                do_unpack(Context, Line, Format, String, Array0, Prev);
            (P, I, Array0, <<Bin:64/float-little, String/binary>>, N, Prev) when Idx =:= <<>> ->
                Array1 = ephp_array:store(I, Bin, Array0),
                PrevStr = <<Prev/binary, Bin:64/float-little>>,
                P(P, I + 1, Array1, String, N - 1, PrevStr);
    ...
    Process(Process, 1, Array, Binary, Size, PrevBin);
do_unpack(Context, Line, [<<A:8, "*", _/binary>> | Format], _Binary, Array, PrevBin)
    when A =:= $x orelse A =:= $@ ->
...
do_unpack(Context, Line, [<<A:8, Rest/binary>> | Format], Binary, Array, PrevBin)
    ...
    case byte_size(Binary) of
        ...
        N when N < Size andalso A =:= $@ ->
            File = ephp_context:get_active_file(Context),
            Data = {<<"unpack">>, <<"@">>},
            ephp_error:handle_error(Context, {error, eoutstr, Line, File, ?E_WARNING, Data}),
...
do_unpack(Context, Line, [<<"X", Rest/binary>> | Format], Binary, Array, PrevBin) ->
    {Size, _} = get_numbers(Rest, 1),
    Process =
        fun (_P, 0, Bin, Prev) ->
                {Bin, Prev};
...
do_pack(Context, Line, <<>>, Args, Binary) ->
    ...
    ephp_error:handle_error(Context, {error, eargunused, Line, File, ?E_WARNING, Data}),
    Binary;
...
do_pack(Context, Line, <<"A", Rest/binary>>, [Arg | Args], Binary) ->
    ...
    String = ephp_data:to_bin(Arg),
    String0 =
        case Size0 - byte_size(String) of
            N when N =:= 0 ->
                <<Binary/binary, String/binary>>;
            N when N < 0 ->
...
do_pack(Context, Line, <<"H", Rest/binary>>, [Arg | Args], Binary) ->
    ...
    String =
        case Size0 - byte_size(ArgStr) of
            N when N =:= 0 ->
...
do_pack(Context, Line, <<C:8, "*", Rest/binary>>, Args, Binary)
    when C =:= $c orelse C =:= $C ->
    String =
        lists:foldl(fun (Arg, S) when is_number(Arg) ->
...
do_pack(Context, Line, <<C:8, Rest/binary>>, Args, Binary)
    ...
    case NSize of
        0 ->
            do_pack(Context, Line, Rest0, NArgs, <<Binary/binary, String/binary>>);
        _ ->
            File = ephp_context:get_active_file(Context),
            Data = {<<"pack">>, <<C:8>>},
            ephp_error:handle_error(Context, {error, efewargs, Line, File, ?E_WARNING, Data}),
            false
    end;
do_pack(Context, Line, <<C:8, "*", Rest/binary>>, Args, Binary)
...
do_pack(Context, Line, <<C:8, "*", Rest/binary>>, Args, Binary)
    when C =:= $i orelse C =:= $I orelse C =:= $l orelse C =:= $L orelse C =:= $V ->
    String =
        lists:foldl(fun (Arg, S) when is_number(Arg) ->
                            A = ephp_data:flooring(Arg),
...
do_pack(Context, Line, <<"X*", Rest/binary>>, Args, Binary) ->
    File = ephp_context:get_active_file(Context),
    Data = {<<"pack">>, <<"X">>, <<"*">>},
    ephp_error:handle_error(Context, {error, ecmdignored, Line, File, ?E_WARNING, Data}),
    do_pack(Context, Line, <<"X", Rest/binary>>, Args, Binary);
do_pack(Context, Line, <<"X", Rest/binary>>, Args, Binary) ->
    {Size0, Rest0} = get_numbers(Rest, 1),
    Result =
        lists:foldl(fun (_, <<>>) ->
                            ...
                            Data = {<<"pack">>, <<"X">>},
                            ephp_error:handle_error(Context,
                                                    {error, eoutstr, Line, File, ?E_WARNING, Data}),
                            undefined;
                        (_, undefined) ->
                            undefined;
                        (_, <<_:1/binary>>) ->
...
do_pack(Context, Line, <<C:1/binary, _/binary>>, _Args, _Binary) ->
    File = ephp_context:get_active_file(Context),
    Data = {<<"pack">>, C},
    ephp_error:handle_error(Context, {error, eunknownfmt, Line, File, ?E_WARNING, Data}),
    <<>>.
...
-spec get_numbers(binary(), binary(), integer()) -> {non_neg_integer(), binary()}.
%% @private
get_numbers(<<>>, <<>>, Default) ->
    {Default, <<>>};
