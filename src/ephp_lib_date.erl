...
-export([init_func/0, init_config/0, init_const/0, handle_error/3, time/2, microtime/2,
         microtime/3, date/3, date/4, gmdate/3, gmdate/4, date_default_timezone_set/3,
         date_default_timezone_get/2, timezone_abbreviations_list/2]).
...
-spec microtime(ephp:context_id(),
                line(),
                GetAsFloat :: {variable() | undefined, boolean()}) ->
                   float() | binary().
...
date(Context, Line, Format) ->
    {MS, S, US} = ephp_datetime:timestamp(),
    date(Context, Line, Format, {undefined, MS * 1000000 + S + trunc(US / 1000000)}).

-spec date(ephp:context_id(), line(), Format :: var_value(), Timestamp :: var_value()) ->
              binary().
...
-spec date_default_timezone_set(ephp:context_id(), line(), TZ :: var_value()) ->
                                   undefined.
...
timezone_abbreviations_list(_Context, _Line) ->
    lists:foldl(fun(Abbr, Array) ->
                   Info =
                       lists:foldl(fun({Dst, Offset, TZID}, SubArray) ->
                                      A0 = ephp_array:new(),
                                   ...
                                   ephp_array:new(),
                                   ephp_timezone:timezone_info(Abbr)),
                   ephp_array:store(
                       ephp_string:to_lower(Abbr), Info, Array)
                end,
                ephp_array:new(),
                ephp_timezone:abbreviations()).

%% ----------------------------------------------------------------------------
%% Internal functions
%% ----------------------------------------------------------------------------

-spec date_format(ToAnalyze :: binary(),
                  Result :: binary(),
                  {Timestamp :: integer(), Date :: calendar:datetime(), TZ :: binary()}) ->
                     binary().
...
date_format(<<"D", R/binary>>, Result, {_, {D, _}, _} = Date) ->
    Day = ephp_datetime:get_abbr_weekday(
...
date_format(<<"S", R/binary>>, Result, {_, {{_, _, D}, _}, _} = Date) ->
    Suffix =
        if D =:= 1 orelse D =:= 21 orelse D =:= 31 ->
               <<"st">>;
           D =:= 2 orelse D =:= 22 ->
               <<"nd">>;
           D =:= 3 orelse D =:= 23 ->
...
date_format(<<"I", R/binary>>, Result, {_, DT, TZ} = Date) ->
    Daylight =
        case ephp_datetime:is_dst(DT, TZ) of
            true ->
                <<"1">>;
            false ->
                <<"0">>
...
date_format(<<"r", R/binary>>,
            Result,
            {_, {{Y, M, D} = Dt, {H, I, S}} = DT, TZ} = Date) ->
    WB = ephp_datetime:get_abbr_weekday(
             calendar:day_of_the_week(Dt)),
    DB = ephp_data:pad_to_bin(D, 2),
