...
-export([start_link/0, destroy/2, get/1, get/2, get_class_name/1, get_class_name/2,
         get_class/1, get_class/2, set_class/2, get_context/1, get_context/2, set/2, set/3, add/2,
         add_link/1, add_link/2, remove_all/2, remove/2, remove/3, clone/2, set_attr/3,
         set_bulk_attr/2]).

set_attr(#obj_ref{pid = Objects, ref = ObjectId} = Object,
         #variable{name = AttrName} = Attr,
         Value) ->
...
set_bulk_attr(#obj_ref{pid = Objects, ref = ObjectId} = Object, Values)
    when is_list(Values) ->
    #ephp_object{context = Ctx, class = Class} = Obj = ephp_object:get(Object),
...
set(Objects, ObjectId, Object) ->
    ObjectsData = erlang:get(Objects),
    NewObjects =
        array:set(ObjectId, Object#ephp_object{id = ObjectId, objects = Objects}, ObjectsData),
...
clone(Context, #obj_ref{pid = Objects, ref = _ObjectId} = ObjRef) ->
    ...
    NewObject =
        Object#ephp_object{context = ClonedContext,
                           id = NewObjectId,
                           links = 1},
