...
-export([function/3, call_args/3, st_function/3, st_use_or_block/3, echo/3, funct_args/3,
         funct_name/3, get_ns/2]).
...
st_use_or_block(<<"{", Rest/binary>>, Parser, Function) ->
    {Rest0, Parser0, CodeBlock} = code(Rest, code_block_level(inc_pos(Parser)), []),
    {Rest0, Parser0, Function#function{code = CodeBlock}};
st_use_or_block(<<U:8, S:8, E:8, SP:8, Rest/binary>>, Parser, Function)
    when ?OR(U, $U, $u)
         andalso ?OR(S, $S, $s)
         andalso ?OR(E, $E, $e)
...
st_function(Rest, Parser, Parsed) ->
    ReturnRef =
        case remove_spaces(Rest, Parser) of
            {<<"&", Rest0/binary>>, Parser0} ->
                true;
            {Rest0, Parser0} ->
                false
...
get_ns({[], Name}, #parser{use_func_list = FuncList} = Parser) ->
    case lists:keyfind(Name, 1, FuncList) of
        {Name, RealNS} ->
            {RealNS, Name};
        false ->
            {ephp_parser:get_ns([], Parser), Name}
    end;
get_ns({NS, Name}, Parser) ->
    {ephp_parser:get_ns(NS, Parser), Name}.

funct_name(<<A:8, Rest/binary>>, Parser, [])
...
funct_args(<<A:8, _/binary>> = Rest, Parser, Parsed) when ?IS_ALPHA(A) orelse A =:= $_ ->
    {Rest0, Parser0, [#constant{name = Constant}]} = ephp_parser:constant(Rest, Parser, []),
...
funct_args(<<"$", _/binary>> = Rest, Parser, Parsed) ->
    ...
    case remove_spaces(Rest0, Parser0) of
        {<<"=", Rest1/binary>>, Parser1} ->
            ...
            {Rest2, Parser2, Default} = ephp_parser_expr:expression(Rest1, NewParser, []),
