...
-export([init_func/0, init_config/0, init_const/0, get_classes/0,
         register_shutdown_function/3, get_defined_functions/2, function_exists/3, func_num_args/2,
         func_get_arg/3, func_get_args/2, call_user_func/3, call_user_func_array/4,
         create_function/4]).
...
get_defined_functions(Context, _Line) ->
    Append =
        fun({Func, Type}, {I, Dict}) ->
           NewTypeDict =
               case ephp_array:find(Type, Dict) of
                   {ok, TypeDict} ->
...
-spec call_user_func_array(ephp:context_id(), line(), var_value(), var_value()) ->
                              mixed().
...
call_user_func(Context, Line, [{_, FuncName} | Args])
    when is_binary(FuncName) orelse ?IS_ARRAY(FuncName) orelse ?IS_OBJECT(FuncName) ->
...
-spec create_function(ephp:context_id(),
                      line(),
                      Args :: var_value(),
                      Code :: var_value()) ->
                         obj_ref().
...
call(Context, Line, Callable, Args) when ?IS_ARRAY(Callable) andalso is_list(Args) ->
    case ephp_array:to_list(Callable) of
        [{_, Object}, {_, Method}] when ?IS_OBJECT(Object) andalso is_binary(Method) ->
            Call =
                #call{name = Method,
                      type = object,
                      args = Args,
