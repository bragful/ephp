...
-export([init_func/0, init_config/0, init_const/0, handle_error/3, include/3,
         include_once/3, require/3, require_once/3]).
...
handle_error(erequired, _Level, {ReqFile, Func}) ->
    ...
    io_lib:format("~s(): Failed opening required '~s' (include_path='~s')",
                  [Func, ReqFile, IncludePath]);
handle_error(einclude, _Level, {ReqFile, Func}) ->
    IncludePath = ephp_config:get(<<"include_path">>, <<".:">>),
    io_lib:format("~s(): Failed opening '~s' for inclusion (include_path='~s')",
                  [Func, ReqFile, IncludePath]);
...
include(Context, Line, {_Var, InclFile}) ->
    ...
    case ephp_context:load(Context, InclFile) of
        {error, _} ->
            ...
            ephp_error:handle_error(Context, {error, enofile, Line, File, ?E_WARNING, NoFileData}),
            ephp_error:handle_error(Context, {error, einclude, Line, File, ?E_WARNING, NoFileData}),
            undefined;
        Code ->
