...
-export([init_func/0, init_config/0, init_const/0, get_classes/0, debug_backtrace/4,
         debug_print_backtrace/4, error_reporting/3, set_error_handler/4, restore_error_handler/2,
         set_exception_handler/3, restore_exception_handler/2, error_get_last/2,
         error_clear_last/2, trigger_error/4]).

-include("ephp.hrl").

-spec init_func() -> ephp_lib:php_function_results().
init_func() ->
    [{debug_backtrace, [{args, {0, 2, undefined, [{str_or_int, 1}, {str_or_int, 0}]}}]},
     {debug_print_backtrace, [{args, {0, 2, undefined, [{str_or_int, 0}, {str_or_int, 0}]}}]},
     {set_error_handler,
     ...
     error_clear_last,
     error_reporting,
     {trigger_error, [{args, {1, 2, false, [string, {integer, ?E_USER_NOTICE}]}}]},
     {trigger_error,
      [{alias, <<"user_error">>}, {args, {1, 2, false, [string, {integer, ?E_USER_NOTICE}]}}]}].
...
-spec debug_print_backtrace(ephp:context_id(), line(), var_value(), var_value()) ->
                               undefined.
debug_print_backtrace(Context, _Line, {_, IncludeArgs}, {_, Limit}) ->
    Backtrace = ephp_stack:get_array(1),
    ephp_array:fold(fun (I, Data, _) when Limit =:= 0 orelse I < Limit ->
...
trace_to_str(I, Array, Ctx, IncludeArgs) ->
    ...
    Args =
        if IncArgs band ?DEBUG_BACKTRACE_IGNORE_ARGS =:= 0 ->
               ArgStrList =
    ...
    io_lib:format("#~p  ~s(~s) called at [~s:~p]~n", [I, FuncName, Args, File, Line]).
...
set_error_handler(Context, Line, {_, ErrorHandler}, {_, ErrorLevel}) ->
    ...
    case ephp_context:is_defined_function(Context, ErrorHandler)
         orelse ?IS_OBJECT(ErrorHandler)
    of
        true ->
            case ephp_error:get_error_handler_func(Context) of
                {OldErrorHandler, _} ->
...
set_exception_handler(Context, Line, {_, ExceptionHandler}) ->
    ...
    case ephp_context:is_defined_function(Context, ExceptionHandler)
         orelse ?IS_OBJECT(ExceptionHandler)
    of
        true ->
...
-spec trigger_error(ephp:context_id(),
                    line(),
                    ErrStr :: var_value(),
                    ErrLevel :: var_value()) ->
                       ephp_error:get_return_return().
trigger_error(Context, Line, _ErrStr, {_, ErrLevel})
    when ErrLevel band bnot ?E_USER =/= 0 ->
    File = ephp_context:get_active_file(Context),
    ephp_error:handle_error(Context, {error, enolevel, Line, File, ?E_WARNING, {}});
trigger_error(Context, Line, {_, ErrStr}, {_, ErrLevel}) ->
    File = ephp_context:get_active_file(Context),
    ephp_error:handle_error(Context, {error, trigger, Line, File, ErrLevel, {ErrStr}}).
