...
-export([init_func/0, init_config/0, init_const/0, phpinfo/3, phpversion/2,
         php_sapi_name/2, php_logo_guid/2, ini_get/3, ini_set/4, set_include_path/3,
         get_include_path/2, version_compare/5, extension_loaded/3, memory_get_usage/3,
...
init_func() ->
    [{phpinfo, [{args, {0, 1, undefined, [{integer, ?INFO_ALL}]}}]},
     php_sapi_name,
     php_logo_guid,
     phpversion,
     ini_get,
     ...
     set_include_path,
     get_include_path,
     {version_compare, [{args, {2, 2, undefined, [string, string, {string, undefined}]}}]},
     {extension_loaded, [{args, {1, 1, undefined, [string]}}]},
     {memory_get_usage, [{args, {0, 1, undefined, [{boolean, false}]}}]},
     {memory_get_peak_usage, [{args, {0, 1, undefined, [{boolean, false}]}}]}].
...
init_const() ->
    [{<<"PHP_VERSION">>, ?PHP_VERSION}, {<<"PHP_MAJOR_VERSION">>, <<?PHP_MAJOR_VERSION>>},
     {<<"PHP_MINOR_VERSION">>, <<?PHP_MINOR_VERSION>>},
     {<<"PHP_RELEASE_VERSION">>, <<?PHP_RELEASE_VERSION>>},
     {<<"PHP_VERSION_ID">>, ?PHP_VERSION_ID}, {<<"PHP_EXTRA_VERSION">>, ?PHP_EXTRA_VERSION},
     % Zend Thread Safety
     {<<"PHP_ZTS">>, 0},
     {<<"PHP_DEBUG">>, 0},
     % TODO: move to ephp_lib_file?
     {<<"PHP_MAXPATHLEN">>, 1024},
     {<<"PHP_OS">>, get_os()}, {<<"PHP_OS_FAMILY">>, get_os_family()},
     {<<"PHP_SAPI">>, ephp_config:get(sapi_type, <<"cli">>)}, {<<"PHP_EOL">>, get_os_eol()},
     % fake:
     {<<"PHP_SHLIB_SUFFIX">>, <<"so">>},
     % assert options:
     {<<"INI_USER">>, 1},
     {<<"INI_PERDIR">>, 2}, {<<"INI_SYSTEM">>, 4}, {<<"INI_ALL">>, 7},
     % phpinfo constants:
     {<<"INFO_GENERAL">>, ?INFO_GENERAL},
     {<<"INFO_CREDITS">>, ?INFO_CREDITS}, {<<"INFO_CONFIGURATION">>, ?INFO_CONFIGURATION},
     {<<"INFO_MODULES">>, ?INFO_MODULES}, {<<"INFO_ENVIRONMENT">>, ?INFO_ENVIRONMENT},
     {<<"INFO_VARIABLES">>, ?INFO_VARIABLES}, {<<"INFO_LICENSE">>, ?INFO_LICENSE},
     {<<"INFO_ALL">>, ?INFO_ALL}].
...
phpinfo(Context, Line, {_, Flags}) ->
    case php_sapi_name(Context, Line) of
        <<"cli", _/binary>> ->
            process_phpinfo("text", Context, Flags);
        _ ->
            process_phpinfo("html", Context, Flags)
    end.
...
-spec version_compare(ephp:context_id(),
                      line(),
                      Vsn1 :: var_value(),
                      Vsn2 :: var_value(),
                      Op :: var_value()) ->
    ...
    Modules =
        lists:map(fun(M) ->
                     case atom_to_binary(M, utf8) of
                         <<"ephp_lib_", Module/binary>> ->
                             Module;
                         Module ->
                             Module
                     end
                  end,
                  ephp_config:get(modules, [])),
...
process_directives({K, V}, Dict) ->
    case binary:split(K, <<".">>) of
        [K] ->
            safe_append(<<"core">>, {K, V}, Dict);
        [Section, _] ->
            safe_append(Section, {K, V}, Dict)
    end.
