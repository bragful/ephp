...
date(Context, Format) ->
    date(Context, Format, time(Context)).

-spec date(Context :: context(), Format :: binary(), Timestamp :: integer()) -> binary().

date(_Context, Format, Timestamp) ->
    M = Timestamp div 1000000,
    S = Timestamp rem 1000000,
    Date = calendar:now_to_local_time({M,S,0}),
    date_format(Format, <<>>, {Timestamp, Date}).
...
date_format(<<"d",R/binary>>, Result, {_,{{_,_,D},_}}=Date) -> 
...
date_format(<<"D",R/binary>>, Result, {_,{D,_}}=Date) ->
    Day = get_abbr_weekday(D),
    date_format(R,<<Result/binary, Day/binary>>, Date);

date_format(<<"j",R/binary>>, Result, {_,{{_,M,_},_}}=Date) ->
    Month = ephp_util:to_bin(M),
    date_format(R,<<Result/binary, Month/binary>>, Date);

date_format(<<"l",R/binary>>, Result, {_,{D,_}}=Date) ->
    Day = get_weekday(D),
    date_format(R,<<Result/binary, Day/binary>>, Date);

date_format(<<"N",R/binary>>, Result, {_,{D,_}}=Date) ->
...
date_format(<<"S",R/binary>>, Result, {_,{{D,_,_},_}}=Date) ->
    Suffix = if
...
date_format(<<"w",R/binary>>, Result, {_,{D,_}}=Date) ->
    Day = calendar:day_of_the_week(D),
    DayBin = ephp_util:to_bin(if
        Day =:= 7 -> 0;
        true -> Day
...
date_format(<<"z",R/binary>>, Result, {_,{{Y,M,D},_}}=Date) ->
...
date_format(<<"W",R/binary>>, Result, {_,{{Y,_,_},_}}=Date) ->
    WeekOfYear = ephp_util:to_bin(calendar:iso_week_number(Y)),
    date_format(R,<<Result/binary, WeekOfYear/binary>>, Date);

date_format(<<"F",R/binary>>, Result, {_,{{_,M,_},_}}=Date) ->
    Month = get_month(M),
    date_format(R,<<Result/binary, Month/binary>>, Date);

date_format(<<"m",R/binary>>, Result, {_,{{_,M,_},_}}=Date) ->
...
date_format(<<"n",R/binary>>, Result, {_,{{_,M,_},_}}=Date) ->
    Month = ephp_util:to_bin(M),
    date_format(R,<<Result/binary, Month/binary>>, Date);

date_format(<<"t",R/binary>>, Result, {_,{{Y,M,_},_}}=Date) ->
    Month = ephp_util:to_bin(calendar:last_day_of_the_month(Y,M)),
    date_format(R,<<Result/binary, Month/binary>>, Date);

date_format(<<"L",R/binary>>, Result, {_,{{Y,_,_},_}}=Date) ->
...
date_format(<<"Y",R/binary>>, Result, {_,{{Y,_,_},_}}=Date) ->
...
date_format(<<"a",R/binary>>, Result, {_,{_,{H,_,_}}}=Date) ->
    Type = if
        H >= 0 andalso H =< 11 -> <<"am">>;
        true -> <<"pm">>
    end,
...
date_format(<<"B",R/binary>>, Result, {_,{_,{H,M,S}}}=Date) ->
    Beats = trunc(((((H * 60) + M) * 60) + S) / 86.4),
    date_format(R,<<Result/binary, Beats/binary>>, Date);

date_format(<<"g",R/binary>>, Result, {_,{_,{H,_,_}}}=Date) ->
...
date_format(<<"h",R/binary>>, Result, {_,{_,{H,_,_}}}=Date) ->
...
date_format(<<"c",R/binary>>, Result, {_,{{Y,M,D},{H,I,S}}}=Date) ->
    DateTime = iolist_to_binary(io_lib:format(
        "~4..0b-~2..0b-~2..0bT~2..0b:~2..0b:~2..0b",
        [Y,M,D,H,I,S])), 
    date_format(R, <<Result/binary,DateTime/binary>>, Date);

date_format(<<"r",R/binary>>, Result, {_,{{Y,M,D}=Dt,{H,I,S}}}=Date) ->
    DateTime = iolist_to_binary(io_lib:format(
        "~w, ~b ~w ~b ~2..0b:~2..0b:~2..0b",
        [get_abbr_weekday(Dt),D,M,Y,H,I,S])),
    date_format(R, <<Result/binary,DateTime/binary>>, Date);

date_format(<<"U",R/binary>>, Result, {Timestamp,_}=Date) ->
    DateTime = ephp_util:to_bin(Timestamp), 
    date_format(R, <<Result/binary,DateTime/binary>>, Date);
