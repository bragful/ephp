...
init_func() ->
    [{in_array, [{args, {2, 3, undefined, [mixed, array, {boolean, false}]}}]},
     ...
     {array_unique, [{args, {1, 2, undefined, [array, {integer, ?SORT_STRING}]}}]},
     {array_change_key_case, [{args, {1, 2, undefined, [array, {integer, ?CASE_LOWER}]}}]},
     {array_chunk, [{args, {2, 3, undefined, [array, integer, {boolean, false}]}}]},
     {array_column, [{args, {2, 3, undefined, [array, mixed, {mixed, undefined}]}}]},
...
-spec array_merge(ephp:context_id(), line(), Arrays :: [var_value()]) ->
                     ephp_array:ephp_array().
array_merge(Context, Line, Args) ->
    array_merge(Context, Line, 1, Args).

-spec list(ephp:context_id(), line(), Vars :: [var_value()]) ->
...
list(Context, _Line, [{Binary, undefined} | Getters]) when is_binary(Binary) ->
    lists:foreach(fun ({[], _}) ->
                          ok;
                      ({G, _}) ->
                          ephp_context:set(Context, G, undefined)
                  end,
                  Getters),
    undefined.

-spec array_unique(ephp:context_id(),
                   line(),
                   Array :: var_value(),
                   Flags :: var_value()) ->
                      ephp_array:ephp_array().
...
-spec array_change_key_case(ephp:context_id(),
                            line(),
                            Array :: var_value(),
                            Flags :: var_value()) ->
                               ephp_array:ephp_array().
array_change_key_case(_Context, _Line, {_, Array}, {_, Flags}) ->
    ephp_array:map(fun(K, V) ->
                      if Flags >= 1 ->
                             {ephp_string:to_upper(K), V};
                         true ->
                             {ephp_string:to_lower(K), V}
                      end
                   end,
                   Array).

-spec array_chunk(ephp:context_id(),
                  line(),
                  Array :: var_value(),
                  Size :: var_value(),
                  PreserveKeys :: var_value()) ->
                     ephp_array:ephp_array().
array_chunk(_Context, _Line, {_, Array}, {_, Size}, {_, PreserveKeys}) ->
    case ephp_array:size(Array) > Size of
        false ->
            Array;
        true ->
            ephp_array:from_list(chunk(ephp_array:to_list(Array), Size, PreserveKeys))
    end.
...
%% @end
array_column(Context, Line, _Array, {_, ColKey}, _IdxKey)
    when not (is_binary(ColKey) orelse is_number(ColKey)) ->
    File = ephp_context:get_active_file(Context),
    Error =
        {error,
         eshouldbe,
         ...
         File,
         ?E_WARNING,
         {<<"array_column">>, <<"The column key">>, <<"either a string or an integer">>}},
    ephp_error:handle_error(Context, Error),
    false;
array_column(Context, Line, _Array, _ColKey, {_, IdxKey})
    when not (is_binary(IdxKey) orelse is_number(IdxKey) orelse IdxKey =:= undefined) ->
    File = ephp_context:get_active_file(Context),
    Error =
        {error,
         eshouldbe,
         ...
         File,
         ?E_WARNING,
         {<<"array_column">>, <<"The index key">>, <<"either a string or an integer">>}},
    ephp_error:handle_error(Context, Error),
    false;
...
-spec reset(ephp:context_id(), line(), Array :: var_value()) ->
               false | ephp_array:ephp_array().
...
-spec current(ephp:context_id(), line(), Array :: var_value()) ->
                 false | ephp_array:ephp_array().
...
-spec php_end(ephp:context_id(), line(), Array :: var_value()) ->
                 false | ephp_array:ephp_array().
...
-spec prev(ephp:context_id(), line(), Array :: var_value()) ->
              false | ephp_array:ephp_array().
...
-spec ksort(ephp:context_id(), line(), Array :: var_value(), SortType :: var_value()) ->
               boolean().
...
-spec array_keys(ephp:context_id(), line(), Array :: var_value()) ->
                    ephp_array:ephp_array().
%% @doc returns a new array with the keys.
array_keys(_Context, _Line, {_, Array}) ->
    ephp_array:keys(Array).

-spec array_pop(ephp:context_id(), line(), Array :: var_value()) ->
                   ephp_array:ephp_array().
...
chunk(Array, N, false) when length(Array) =< N ->
    {_, NewArray} =
        lists:foldl(fun({_, V}, {K, Res}) -> {K + 1, Res ++ [{K, V}]} end, {0, []}, Array),
...
unique([{Key, Val} | Rest], Array, ?SORT_REGULAR) ->
    Check = fun({_, V}) -> ephp_data:is_equal(V, Val) end,
    case lists:any(Check, Array) of
        true ->
            unique(Rest, Array, ?SORT_REGULAR);
        false ->
            unique(Rest, Array ++ [{Key, Val}], ?SORT_REGULAR)
    end;
unique([{Key, Val} | Rest], Array, ?SORT_NUMERIC) ->
    NumArray = [{K, ephp_data:to_number(V)} || {K, V} <- Array],
    NumVal = ephp_data:to_number(Val),
    Check = fun({_, V}) -> V == NumVal end,
    case lists:any(Check, NumArray) of
        true ->
            unique(Rest, Array, ?SORT_NUMERIC);
        false ->
            unique(Rest, Array ++ [{Key, Val}], ?SORT_NUMERIC)
    end;
unique([{Key, Val} | Rest], Array, _Flags) ->
    ...
    case lists:keyfind(StrVal, 2, StrArray) of
        {_, StrVal} ->
            unique(Rest, Array, ?SORT_STRING);
        false ->
            unique(Rest, Array ++ [{Key, Val}], ?SORT_STRING)
    end.
...
member(Value, Dict, false) ->
    List = ephp_array:to_list(Dict),
    lists:any(fun({_, Member}) -> ephp_data:is_equal(Member, Value) end, List).
...
array_merge(Context, Line, N, [{V1, A1}, {_, A2} | Rest]) when ?IS_ARRAY(A2) ->
    Array =
        lists:foldl(fun ({K, V}, A) when is_integer(K) ->
                            ephp_array:store(auto, V, A);
                        ({K, V}, A) ->
                            ephp_array:store(K, V, A)
                    end,
                    A1,
                    ephp_array:to_list(A2)),
