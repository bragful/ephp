...
-export([start_link/0, start_link/1, stop/0, calls/1, erase/1]).
...
init([OutputFun]) ->
    {ok, #{filters => [], output => OutputFun}}.

handle_cast(#stack_trace{} = StackTrace, #{filters := Filters} = State) ->
    Options =
        [{function, key_name(StackTrace)}, {file, filename:absname(StackTrace#stack_trace.file)}],
...
handle_cast({calls, Name}, #{filters := Filters} = State) ->
    case lists:member(Name, Filters) of
        true ->
            {noreply, State};
        false ->
            {noreply, State#{filters => [Name | Filters]}}
    end.
