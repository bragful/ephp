...
-export([add_pos/2, new_line/1, copy_rowcol/2, add_line/2, remove_spaces/2, throw_error/3,
         code_block/3, code/3, new_line/2, inc_pos/1, get_ns/2, array_def_level/1,
         code_statement_level/1, arg_level/1, code_block_level/1, enclosed_level/1,
         unclosed_level/1, get_line/1, variable/3, comment_line/3, comment_block/3, constant/3]).

-include("ephp.hrl").
-include("ephp_parser.hrl").

-import(ephp_parser_expr, [expression/3, add_op/2, precedence/1]).
...
code_ns(<<N:8, A:8, M:8, E:8, S:8, P:8, A:8, C:8, E:8, SP:8, Rest/binary>>,
    ...
    when ?OR(N, $N, $n)
         ...
         andalso NScanBe =/= false ->
    {Rest0, Parser0, NameSpace} = namespace(<<SP:8, Rest/binary>>, add_pos(Parser, 9), []),
    case remove_spaces(Rest0, Parser0#parser{namespace = NameSpace, namespace_can_be = false})
    of
        {<<";", _/binary>> = Rest1, Parser1}
            when NScanBe =:= true orelse NScanBe =:= only_statement ->
            code(Rest1, Parser1#parser{namespace_can_be = only_statement}, Parsed);
        ...
        {<<"{", _/binary>> = Rest1, Parser1}
            when NScanBe =:= only_block orelse NScanBe =:= true ->
            {Rest2, Parser2, CodeBlock} = code_block(Rest1, Parser1, []),
            Parser2_1 = Parser2#parser{namespace = [], namespace_can_be = only_block},
...
code(<<B:8, R:8, E:8, A:8, K:8, SP:8, Rest/binary>>, Parser, Parsed)
    when ?OR(B, $B, $b)
         andalso ?OR(R, $R, $r)
         andalso ?OR(E, $E, $e)
         andalso ?OR(A, $A, $a)
         andalso ?OR(K, $K, $k)
         andalso not (?IS_SPACE(SP) orelse ?IS_NUMBER(SP) orelse SP =:= $_) ->
    code(<<SP:8, Rest/binary>>, add_pos(Parser, 5), [break | Parsed]);
code(<<C:8, O:8, N:8, T:8, I:8, N:8, U:8, E:8, SP:8, Rest/binary>>, Parser, Parsed)
    when ?OR(C, $C, $c)
         andalso ?OR(O, $O, $o)
         andalso ?OR(N, $N, $n)
         andalso ?OR(T, $T, $t)
         ...
         andalso ?OR(U, $U, $u)
         andalso ?OR(E, $E, $e)
         andalso not (?IS_SPACE(SP) orelse ?IS_NUMBER(SP)) ->
    code(<<SP:8, Rest/binary>>, add_pos(Parser, 8), [continue | Parsed]);
code(<<R:8, E:8, T:8, U:8, R:8, N:8, SP:8, Rest/binary>>, Parser, Parsed)
    when ?OR(R, $R, $r)
         andalso ?OR(E, $E, $e)
         andalso ?OR(T, $T, $t)
         andalso ?OR(U, $U, $u)
    ...
    case Return of
        ...
        _ ->
            code(Rest0, Parser0, [add_line(#return{value = Return}, Parser) | Parsed])
    end;
code(<<T:8, H:8, R:8, O:8, W:8, SP:8, Rest/binary>>, Parser, Parsed)
    when ?OR(T, $T, $t)
         andalso ?OR(H, $H, $h)
         andalso ?OR(R, $R, $r)
         andalso ?OR(O, $O, $o)
    ...
    case Throw of
        ...
        _ ->
            code(Rest0, Parser0, [add_line(#throw{value = Throw}, Parser) | Parsed])
    end;
code(<<T:8, R:8, Y:8, SP:8, Rest/binary>>, Parser, Parsed)
    when ?OR(T, $T, $t)
         andalso ?OR(R, $R, $r)
         andalso ?OR(Y, $Y, $y)
         andalso not (?IS_ALPHA(SP) orelse ?IS_NUMBER(SP) orelse SP =:= $_) ->
...
code(<<F:8, I:8, N:8, A:8, L:8, L:8, Y:8, SP:8, Rest/binary>>,
     Parser,
     [#try_catch{} = Try | Parsed])
    when ?OR(F, $F, $f)
         andalso ?OR(I, $I, $i)
...
code(<<G:8, L:8, O:8, B:8, A:8, L:8, SP:8, Rest/binary>>, Parser, Parsed)
    when ?OR(G, $G, $g)
         andalso ?OR(L, $L, $l)
         andalso ?OR(O, $O, $o)
         andalso ?OR(B, $B, $b)
...
code(<<E:8, N:8, D:8, I:8, F:8, SP:8, Rest/binary>>,
     #parser{level = if_old_block} = Parser,
     Parsed)
    when ?OR(E, $E, $e)
         andalso ?OR(N, $N, $n)
         andalso ?OR(D, $D, $d)
...
code(<<E:8, N:8, D:8, S:8, W:8, I:8, T:8, C:8, H:8, SP:8, Rest/binary>>,
     #parser{level = switch_old_block} = Parser,
     Parsed)
    when ?OR(E, $E, $e)
         andalso ?OR(N, $N, $n)
         andalso ?OR(D, $D, $d)
...
code(<<E:8, N:8, D:8, F:8, O:8, R:8, E:8, A:8, C:8, H:8, SP:8, Rest/binary>>,
     #parser{level = foreach_old_block} = Parser,
     Parsed)
    when ?OR(E, $E, $e)
         andalso ?OR(N, $N, $n)
         andalso ?OR(D, $D, $d)
...
code(<<E:8, N:8, D:8, F:8, O:8, R:8, SP:8, Rest/binary>>,
     #parser{level = for_old_block} = Parser,
     Parsed)
    when ?OR(E, $E, $e)
         andalso ?OR(N, $N, $n)
         andalso ?OR(D, $D, $d)
...
code(<<E:8, N:8, D:8, W:8, H:8, I:8, L:8, E:8, SP:8, Rest/binary>>,
     #parser{level = while_old_block} = Parser,
     Parsed)
    when ?OR(E, $E, $e)
         andalso ?OR(N, $N, $n)
         andalso ?OR(D, $D, $d)
...
code(<<I:8, F:8, SP:8, Rest/binary>>, Parser, Parsed)
    when ?OR(I, $i, $I) andalso ?OR(F, $f, $F) andalso (?IS_SPACE(SP) orelse SP =:= $() ->
...
code(<<D:8, O:8, SP:8, Rest/binary>>, Parser, Parsed)
    when ?OR(D, $d, $D)
         andalso ?OR(O, $o, $O)
         andalso (?IS_SPACE(SP) orelse ?OR(SP, ${, $:) orelse ?IS_NEWLINE(SP)) ->
...
code(<<F:8, O:8, R:8, SP:8, Rest/binary>>, Parser, Parsed)
    ...
    code(Rest1, copy_rowcol(Parser1, Parser), NewParsed);
code(<<E:8, L:8, S:8, E:8, SP:8, _/binary>> = Rest,
     #parser{level = if_old_block} = Parser,
     Parsed)
    when ?OR(E, $e, $E)
         andalso ?OR(L, $l, $L)
    ...
    {Rest, Parser, Parsed};
code(<<S:8, W:8, I:8, T:8, C:8, H:8, SP:8, Rest/binary>>, Parser, Parsed)
    when ?OR(S, $S, $s)
         andalso ?OR(W, $W, $w)
         andalso ?OR(I, $I, $i)
         andalso ?OR(T, $T, $t)
    ...
    code(Rest1, copy_rowcol(Parser1, Parser), NewParsed);
code(<<C:8, A:8, S:8, E:8, SP:8, Rest/binary>>, #parser{level = Level} = Parser, Parsed)
    when (Level =:= switch_block orelse Level =:= switch_old_block)
         andalso ?OR(C, $C, $c)
         andalso ?OR(A, $A, $a)
         andalso ?OR(S, $S, $s)
         andalso ?OR(E, $E, $e)
    ...
    case expression(Rest0, NewParser, []) of
        {<<":", Rest1/binary>>, Parser1, Exp} ->
            ok;
        {<<";", Rest1/binary>>, Parser1, Exp} ->
            ok
    end,
    NewParsed =
        [add_line(#switch_case{label = Exp, code_block = []}, Parser)
         | switch_case_block(Parsed)],
    code(Rest1, copy_rowcol(inc_pos(Parser1), Parser), NewParsed);
code(<<C:8, A:8, S:8, E:8, SP:8, Rest/binary>>,
     #parser{level = code_statement} = Parser,
     Parsed)
    when ?OR(C, $C, $c)
         andalso ?OR(A, $A, $a)
         andalso ?OR(S, $S, $s)
...
code(<<D:8, E:8, F:8, A:8, U:8, L:8, T:8, SP:8, Rest/binary>>,
    ...
    NewParsed =
        [add_line(#switch_case{label = default, code_block = []}, Parser)
         | switch_case_block(Parsed)],
...
code(<<A:8, B:8, S:8, T:8, R:8, A:8, C:8, T:8, SP:8, Rest/binary>>, Parser, Parsed)
    when ?OR(A, $A, $a)
         andalso ?OR(B, $B, $b)
         andalso ?OR(S, $S, $s)
         andalso ?OR(T, $T, $t)
...
code(<<F:8, I:8, N:8, A:8, L:8, SP:8, Rest/binary>>, Parser, Parsed)
    when ?OR(F, $F, $f)
         andalso ?OR(I, $I, $i)
         andalso ?OR(N, $N, $n)
         andalso ?OR(A, $A, $a)
...
code(<<C:8, L:8, A:8, S:8, S:8, SP:8, Rest/binary>>, Parser, Parsed)
    when ?OR(C, $C, $c)
         andalso ?OR(L, $L, $l)
         andalso ?OR(A, $A, $a)
         andalso ?OR(S, $S, $s)
         andalso (?IS_SPACE(SP) orelse ?IS_NEWLINE(SP)) ->
    {Rest0, Parser0, Class0} =
        case Parsed of
            [#class{name = undefined} = Class | _] ->
                ephp_parser_class:st_class(<<SP:8, Rest/binary>>, add_pos(Parser, 5), Class);
            _ ->
                Class = add_line(#class{}, Parser),
                ephp_parser_class:st_class(<<SP:8, Rest/binary>>, add_pos(Parser, 5), Class)
        end,
    code(Rest0, copy_rowcol(Parser0, Parser), [Class0 | Parsed]);
code(<<U:8, S:8, E:8, SP:8, Rest/binary>>, Parser, Parsed)
    when ?OR(U, $U, $u)
         andalso ?OR(S, $S, $s)
         andalso ?OR(E, $E, $e)
         andalso (?IS_SPACE(SP) orelse ?IS_NEWLINE(SP)) ->
    case remove_spaces(<<SP:8, Rest/binary>>, add_pos(Parser, 3)) of
        {<<C:8, O:8, N:8, S:8, T:8, SP:8, Rest0/binary>>, Parser0}
            when ?OR(C, $C, $c)
                 andalso ?OR(O, $O, $o)
                 andalso ?OR(N, $N, $n)
                 andalso ?OR(S, $S, $s)
        ...
        {<<F:8, U:8, N:8, C:8, T:8, I:8, O:8, N:8, SP:8, Rest0/binary>>, Parser0}
            when ?OR(F, $F, $f)
                 andalso ?OR(U, $U, $u)
                 andalso ?OR(N, $N, $n)
                 andalso ?OR(C, $C, $c)
...
code(<<N:8, A:8, M:8, E:8, S:8, P:8, A:8, C:8, E:8, SP:8, _/binary>> = Rest,
     #parser{namespace_can_be = only_statement} = Parser,
     Parsed)
    when ?OR(N, $N, $n)
         andalso ?OR(A, $A, $a)
         andalso ?OR(M, $M, $m)
...
code(<<N:8, A:8, M:8, E:8, S:8, P:8, A:8, C:8, E:8, SP:8, _/binary>>, Parser, _Parsed)
    when ?OR(N, $N, $n)
         andalso ?OR(A, $A, $a)
         andalso ?OR(M, $M, $m)
         andalso ?OR(E, $E, $e)
...
code(<<I:8, N:8, T:8, E:8, R:8, F:8, A:8, C:8, E:8, SP:8, Rest/binary>>, Parser, Parsed)
    when ?OR(I, $I, $i)
         andalso ?OR(N, $N, $n)
         andalso ?OR(T, $T, $t)
         andalso ?OR(E, $E, $e)
    ...
    {Rest0, Parser0, Interface0} =
        ephp_parser_class:st_interface(<<SP:8, Rest/binary>>, add_pos(Parser, 9), Interface),
    code(Rest0, copy_rowcol(Parser0, Parser), [Interface0 | Parsed]);
code(<<E:8, C:8, H:8, O:8, SP:8, Rest/binary>>, Parser, Parsed)
    when ?OR(E, $e, $E)
         andalso ?OR(C, $c, $C)
         andalso ?OR(H, $h, $H)
         andalso ?OR(O, $o, $O)
    ...
    case Call0#call.args of
        [#operation{type = Type} = A1] when Type =:= 'or' orelse Type =:= 'and' ->
            Print = A1#operation{expression_left = get_print(A1#operation.expression_left, Parser)},
...
code(<<F:8, U:8, N:8, C:8, T:8, I:8, O:8, N:8, SP:8, Rest/binary>>, Parser, Parsed)
    when ?OR(F, $F, $f)
         andalso ?OR(U, $U, $u)
         andalso ?OR(N, $N, $n)
         andalso ?OR(C, $C, $c)
...
code(<<"?>", Rest/binary>>, #parser{level = code_value} = Parser, [Parsed]) ->
    {Rest, add_pos(Parser, 2), Parsed};
code(<<"?>\n", Rest/binary>>, #parser{level = L} = Parser, Parsed)
    when L =:= code_block
         orelse L =:= if_old_block
         orelse L =:= while_old_block
         orelse L =:= for_old_block
         orelse L =:= foreach_old_block
         orelse L =:= switch_block
         orelse L =:= switch_old_block ->
...
code(<<"?>", Rest/binary>>, #parser{level = L} = Parser, Parsed)
    when L =:= code_block
         orelse L =:= if_old_block
         orelse L =:= while_old_block
         orelse L =:= for_old_block
...
code(<<I:8, N:8, C:8, L:8, U:8, D:8, E:8, SP:8, Rest/binary>>, Parser, Parsed)
    when ?OR(I, $I, $i)
         andalso ?OR(N, $N, $n)
         andalso ?OR(C, $C, $c)
         andalso ?OR(L, $L, $l)
...
code(<<I:8, N:8, C:8, L:8, U:8, D:8, E:8, $_, O:8, N:8, C:8, E:8, SP:8, Rest/binary>>,
    ...
    code(Rest1, Parser1, [Include | Parsed]);
code(<<R:8, E:8, Q:8, U:8, I:8, R:8, E:8, SP:8, Rest/binary>>, Parser, Parsed)
    when ?OR(R, $R, $r)
         andalso ?OR(E, $E, $e)
         andalso ?OR(Q, $Q, $q)
         andalso ?OR(U, $U, $u)
...
code(<<R:8, E:8, Q:8, U:8, I:8, R:8, E:8, $_, O:8, N:8, C:8, E:8, SP:8, Rest/binary>>,
    ...
    code(Rest1, Parser1, [Include | Parsed]);
code(<<S:8, T:8, A:8, T:8, I:8, C:8, SP:8, Rest/binary>>, Parser, Parsed)
    when ?OR(S, $S, $s)
         andalso ?OR(T, $T, $t)
         andalso ?OR(A, $A, $a)
         andalso ?OR(I, $I, $i)
...
code(<<A:8, _/binary>> = Rest, Parser, [#constant{} | _])
    ...
    throw_error(eparse, Parser, Rest);
code(<<A:8, _/binary>> = Rest, Parser, Parsed)
    when ?IS_ALPHA(A) orelse A =:= $_ orelse A =:= $\\ ->
...
get_ns([], #parser{namespace = BaseNS}) ->
    BaseNS;
get_ns([<<>>], _Parser) ->
    [];
get_ns([FirstName | RestNames] = NS, #parser{use_list = UseList, namespace = BaseNS}) ->
    case ephp_string:to_lower(FirstName) of
        ...
        _ ->
            ...
            case ephp_ns:find(NS, UseList) of
                {MatchNS, RealNS} ->
                    ephp_ns:translate(NS, MatchNS, RealNS);
                false ->
                    FullNS
            end
    end.

use_const(Rest, #parser{use_const_list = ConstList, use_ns = UseNS} = Parser, Parsed) ->
    ...
    case remove_spaces(Rest0, Parser0) of
        ...
        {<<"}", Rest1/binary>>, #parser{use_ns = UseNS} = Parser1} when UseNS =/= [] ->
...
use_function(Rest, #parser{use_func_list = FuncList, use_ns = UseNS} = Parser, Parsed) ->
    {Rest0, Parser0, NameSpace} = namespace(Rest, Parser, []),
    {NS, BaseName} = ephp_ns:split(NameSpace),
    BaseNS = ephp_ns:join(UseNS, NS),
    case remove_spaces(Rest0, Parser0) of
...
use_list(Rest, #parser{use_ns = UseNS, use_list = UseList} = Parser, Parsed) ->
    ...
    case remove_spaces(Rest0, Parser0) of
        ...
        {<<A:8, S:8, SP:8, Rest1/binary>>, Parser1}
            when ?OR(A, $A, $a)
                 andalso ?OR(S, $S, $s)
                 andalso (?IS_SPACE(SP) orelse ?IS_NEWLINE(SP)) ->
            {Rest2, Parser2, AliasNS} = namespace(<<SP:8, Rest1/binary>>, add_pos(Parser1, 2), []),
            NewUseList = [{AliasNS, BaseNS} | UseList],
...
namespace(<<A:8, _/binary>> = Rest, Parser, []) when ?IS_ALPHANUM(A) orelse A =:= $_ ->
    namespace(Rest, Parser, [<<>>]);
namespace(<<A:8, Rest/binary>>, Parser, [NamePart | Parsed])
...
variable(<<SP:8, Rest/binary>>, #parser{level = enclosed} = Parser, Var)
    when ?IS_SPACE(SP) ->
...
variable(<<A:8, Rest/binary>>, #parser{} = Parser, [#variable{name = N} = V])
    when ?IS_NUMBER(A) orelse ?IS_ALPHA(A) orelse A =:= $_ orelse A >= 16#7f ->
...
accessor(<<"{", Rest/binary>>, Parser, []) ->
    ...
    case remove_spaces(Rest0, Parser0) of
        {<<"(", Rest1/binary>>, Parser1} ->
            ...
            {Rest2,
             copy_rowcol(Parser2, Parser),
             [add_line(#call{name = Acc,
                             args = Args,
...
constant(<<A:8, Rest/binary>>, Parser, [#constant{name = N} = C])
    ...
    constant(Rest, inc_pos(Parser), [C#constant{name = <<N/binary, A:8>>}]);
constant(<<SP:8, _/binary>> = Rest,
         #parser{level = unclosed} = Parser,
         [#constant{}] = Parsed)
    when ?IS_SPACE(SP) ->
    {Rest, Parser, Parsed};
constant(<<SP:8, Rest/binary>>, Parser, [#constant{}] = Parsed) when ?IS_SPACE(SP) ->
...
constant(<<SP:8, _/binary>> = Rest,
    ...
    when ?IS_NEWLINE(SP) ->
    {Rest, Parser, Parsed};
constant(<<SP:8, Rest/binary>>, Parser, [#constant{}] = Parsed) when ?IS_NEWLINE(SP) ->
...
constant_wait(<<"(", Rest/binary>>, Parser, [#constant{} = C]) ->
    {NS, Name} =
        ephp_parser_func:get_ns(
            ephp_ns:parse(C#constant.name), Parser),
...
constant_wait(<<SP:8, Rest/binary>>, Parser, [#constant{}] = Parsed) when ?IS_SPACE(SP) ->
    constant_wait(Rest, inc_pos(Parser), Parsed);
...
constant_known([#constant{name = RawName} = C | Parsed], Parser) ->
    ...
    case lists:member(C#constant.name, ephp_const:special_consts()) of
        true ->
            [C | Parsed];
        false ->
            [C#constant{namespace = NS, name = Name} | Parsed]
    end.
...
st_while(<<"(", Rest/binary>>, Parser, Parsed) ->
    ...
    {<<")", Rest1/binary>>, Parser1, Conditions} = expression(Rest, arg_level(NewParser), []),
    {Rest2, Parser2, CodeBlock} = code_block(Rest1, while_block_level(Parser1), []),
    While =
        add_line(#while{type = pre,
...
st_do_while(Rest, Parser, Parsed) ->
    case code_block(Rest, Parser, []) of
        {<<";", Rest0/binary>>, Parser0, CodeBlock} ->
            ok;
    ...
    DoWhile = add_line(While#while{type = post, loop_block = CodeBlock}, Parser),
...
st_if(<<"(", Rest/binary>>, Parser, []) ->
    ...
    If = add_line(#if_block{conditions = Conditions, true_block = CodeBlock}, Parser),
    st_if(Rest2, copy_rowcol(Parser2, Parser), [If]);
st_if(<<E:8, L:8, S:8, E:8, SP:8, Rest/binary>>, Parser, [#if_block{} = If | Parsed])
    when ?OR(E, $e, $E)
         andalso ?OR(L, $l, $L)
         andalso ?OR(S, $s, $S)
         andalso (?OR(SP, ${, $:) orelse ?IS_SPACE(SP) orelse ?IS_NEWLINE(SP)) ->
    {Rest0, Parser0} = remove_spaces(<<SP:8, Rest/binary>>, add_pos(Parser, 4)),
...
st_if(<<E:8, L:8, S:8, E:8, I:8, F:8, SP:8, Rest/binary>>,
      Parser,
      [#if_block{} = If | Parsed])
    when ?OR(E, $e, $E)
         andalso ?OR(L, $l, $L)
...
switch_case_block(Blocks) ->
    {Block, [Switch | Rest]} =
        lists:splitwith(fun (#switch_case{}) ->
                                false;
                            (_) ->
                                true
                        end,
...
st_switch(<<"(", Rest/binary>>, Parser, Parsed) ->
    ...
    NewParser =
        case remove_spaces(Rest0, inc_pos(Parser0)) of
            {<<"{", Rest1/binary>>, Parser1} ->
...
st_for(<<"(", Rest/binary>>, Parser, Parsed) ->
    ...
    {Rest3, Parser3, CodeBlock} = code_block(Rest2, for_block_level(inc_pos(Parser2)), []),
    ForCodeBlock = CodeBlock ++ Upda,
    For = add_line(#while{type = post,
                          conditions = Cond,
                          loop_block = ForCodeBlock},
                   Parser),
...
set_level(Level, Parser) ->
    Parser#parser{level = Level}.
...
if_old_block_level(Parser) ->
    set_level(if_old_block, Parser).

for_old_block_level(Parser) ->
    set_level(for_old_block, Parser).

foreach_old_block_level(Parser) ->
    set_level(foreach_old_block, Parser).

while_old_block_level(Parser) ->
    set_level(while_old_block, Parser).

switch_old_block_level(Parser) ->
    set_level(switch_old_block, Parser).

if_block_level(Parser) ->
    set_level(if_block, Parser).

for_block_level(Parser) ->
    set_level(for_block, Parser).

foreach_block_level(Parser) ->
    set_level(foreach_block, Parser).

while_block_level(Parser) ->
    set_level(while_block, Parser).

switch_block_level(Parser) ->
    set_level(switch_block, Parser).

switch_label_level(Parser) ->
    set_level(switch_label, Parser).

normal_level(Parser) ->
    set_level(code, Parser).

code_block_level(Parser) ->
    set_level(code_block, Parser).

code_value_level(Parser) ->
    set_level(code_value, Parser).

code_statement_level(Parser) ->
    set_level(code_statement, Parser).

arg_level(Parser) ->
    set_level(arg, Parser).

array_level(Parser) ->
    set_level(array, Parser).

array_curly_level(Parser) ->
    set_level(array_curly, Parser).

array_def_level(Parser) ->
    (set_level(array_def, Parser))#parser{array_type = old}.

literal_level(Parser) ->
    set_level(literal, Parser).

enclosed_level(Parser) ->
    set_level(enclosed, Parser).

unclosed_level(Parser) ->
    set_level(unclosed, Parser).

get_line(#parser{row = Row, col = Col}) ->
    {{line, Row}, {column, Col}};
get_line(#array{line = Line}) ->
    Line;
get_line(#eval{line = Line}) ->
    Line;
get_line(#print{line = Line}) ->
    Line;
get_line(#print_text{line = Line}) ->
    Line;
get_line(#variable{line = Line}) ->
    Line;
get_line(#constant{line = Line}) ->
    Line;
get_line(#int{line = Line}) ->
    Line;
get_line(#float{line = Line}) ->
    Line;
get_line(#text_to_process{line = Line}) ->
    Line;
get_line(#text{line = Line}) ->
    Line;
get_line(#if_block{line = Line}) ->
    Line;
get_line(#assign{line = Line}) ->
    Line;
get_line(#array_element{line = Line}) ->
    Line;
get_line(#foreach{line = Line}) ->
    Line;
get_line(#operation{line = Line}) ->
    Line;
get_line(#concat{line = Line}) ->
    Line;
get_line(#while{line = Line}) ->
    Line;
get_line(#return{line = Line}) ->
    Line;
get_line(#function{line = Line}) ->
    Line;
get_line(#global{line = Line}) ->
    Line;
get_line(#ref{line = Line}) ->
    Line;
get_line(#switch{line = Line}) ->
    Line;
get_line(#switch_case{line = Line}) ->
    Line;
get_line(#call{line = Line}) ->
    Line;
get_line(#class{line = Line}) ->
    Line;
get_line(#class_method{line = Line}) ->
    Line;
get_line(#class_const{line = Line}) ->
    Line;
get_line(#class_attr{line = Line}) ->
    Line;
get_line(#instance{line = Line}) ->
    Line;
get_line(#cast{line = Line}) ->
    Line;
get_line(#throw{line = Line}) ->
    Line;
get_line(#try_catch{line = Line}) ->
    Line;
get_line(#catch_block{line = Line}) ->
    Line;
get_line(#clone{line = Line}) ->
    Line;
get_line(#command{line = Line}) ->
    Line.
...
add_line(false, _) ->
    false;
add_line(X, {{line, Row}, {column, Col}}) ->
    add_line(X, #parser{row = Row, col = Col});
add_line(#array{} = A, Parser) ->
    A#array{line = get_line(Parser)};
add_line(#eval{} = E, Parser) ->
    E#eval{line = get_line(Parser)};
add_line(#print{} = P, Parser) ->
    P#print{line = get_line(Parser)};
add_line(#print_text{} = P, Parser) ->
    P#print_text{line = get_line(Parser)};
add_line(#variable{} = V, Parser) ->
    V#variable{line = get_line(Parser)};
add_line(#constant{} = O, Parser) ->
    O#constant{line = get_line(Parser)};
add_line(#int{} = I, Parser) ->
    I#int{line = get_line(Parser)};
add_line(#float{} = F, Parser) ->
    F#float{line = get_line(Parser)};
add_line(#php_n{} = N, Parser) ->
    N#php_n{line = get_line(Parser)};
add_line(#text_to_process{} = T, Parser) ->
    T#text_to_process{line = get_line(Parser)};
add_line(#text{} = T, Parser) ->
    T#text{line = get_line(Parser)};
add_line(#if_block{} = I, Parser) ->
    I#if_block{line = get_line(Parser)};
add_line(#assign{} = A, Parser) ->
    A#assign{line = get_line(Parser)};
add_line(#array_element{} = A, Parser) ->
    A#array_element{line = get_line(Parser)};
add_line(#foreach{} = F, Parser) ->
    F#foreach{line = get_line(Parser)};
add_line(#operation{} = O, Parser) ->
    O#operation{line = get_line(Parser)};
add_line(#concat{} = O, Parser) ->
    O#concat{line = get_line(Parser)};
add_line(#while{} = W, Parser) ->
    W#while{line = get_line(Parser)};
add_line(#return{} = Rt, Parser) ->
    Rt#return{line = get_line(Parser)};
add_line(#function{} = F, Parser) ->
    F#function{line = get_line(Parser)};
add_line(#global{} = G, Parser) ->
    G#global{line = get_line(Parser)};
add_line(#ref{} = Rf, Parser) ->
    Rf#ref{line = get_line(Parser)};
add_line(#switch{} = S, Parser) ->
    S#switch{line = get_line(Parser)};
add_line(#switch_case{} = S, Parser) ->
    S#switch_case{line = get_line(Parser)};
add_line(#call{} = Cl, Parser) ->
    Cl#call{line = get_line(Parser)};
add_line(#class{} = Cl, Parser) ->
    Cl#class{line = get_line(Parser)};
...
add_line({object, Expr}, Parser) ->
    {object, Expr, get_line(Parser)};
add_line({class, Expr}, Parser) ->
    {class, Expr, get_line(Parser)};
add_line(#instance{} = I, Parser) ->
    I#instance{line = get_line(Parser)};
add_line(#cast{} = Cs, Parser) ->
    Cs#cast{line = get_line(Parser)};
add_line(#throw{} = T, Parser) ->
    T#throw{line = get_line(Parser)};
add_line(#try_catch{} = T, Parser) ->
    T#try_catch{line = get_line(Parser)};
add_line(#catch_block{} = B, Parser) ->
    B#catch_block{line = get_line(Parser)};
add_line(#clone{} = L, Parser) ->
    L#clone{line = get_line(Parser)};
add_line(#command{} = Co, Parser) ->
    Co#command{line = get_line(Parser)}.
...
remove_spaces(<<>>, Parser) ->
    {<<>>, Parser};
remove_spaces(Rest, Parser) ->
    {Rest, Parser}.

get_print({Type, Value, _}, Parser) when Type =:= int; Type =:= float; Type =:= text ->
