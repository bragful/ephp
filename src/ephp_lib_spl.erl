...
-export([init_func/0, init_config/0, init_const/0, spl_autoload_call/3,
         spl_autoload_register/5, spl_autoload_unregister/3]).
...
init_func() ->
    [{spl_autoload_call, [{args, [string]}]},
     {spl_autoload_register, [{args, [callable, {boolean, false}, {boolean, false}]}]},
...
spl_autoload_call(Context, Line, {_, RawClassName}) ->
    ...
    case ephp_class:get(Context, ClassNS, ClassName, spl) of
        {ok, _Class} ->
            undefined;
        {error, enoexist} ->
            ephp_stack:push(undefined, Line, <<"spl_autoload">>, [ClassName], undefined, undefined),
            Classes = ephp_context:get_classes(Context),
            ExceptionName = <<"LogicException">>,
            Exception = ephp_class:instance(Classes, Context, Context, [], ExceptionName, Line),
            #ephp_object{class = Class} = ephp_object:get(Exception),
            #class_method{name = ConstructorName} = ephp_class:get_constructor(Classes, Class),
            Call =
                #call{type = object,
                      name = ConstructorName,
                      args = [<<"Class ", ClassName/binary, " could not be loaded">>],
                      line = Line},
...
-spec spl_autoload_register(ephp:context_id(),
                            line(),
                            var_value(),
                            var_value(),
                            var_value()) ->
                               boolean().
spl_autoload_register(Context, _Line, {_, Function}, {_, _Thrown}, {_, RawPrepend}) ->
