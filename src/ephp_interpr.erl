...
-export([process/2, process/3, run/2, run/3]).
...
process(Context, Statements, false) ->
    Value =
        try
            lists:foldl(fun (Statement, false) ->
...
-spec run_depth(ephp:context_id(), statement(), flow_status(), Cover :: boolean()) ->
                   flow_status().
...
run_depth(Context, {Op, _Var, Line} = MonoArith, false, Cover)
    when Op =:= pre_incr
         orelse Op =:= pre_decr
         orelse Op =:= post_incr
         orelse Op =:= post_decr ->
...
run_depth(Context, #operation{line = Line} = Op, false, Cover) ->
    ...
    false;
run_depth(Context, #class{type = interface, line = Line} = Interface, Return, Cover) ->
...
run_depth(Context, {silent, Statement}, false, Cover) ->
    ephp_error:run_quiet(
        ephp_context:get_errors_id(Context),
        fun() -> run_depth(Context, Statement, false, Cover) end);
run_depth(_Context, die, false, _Cover) ->
    throw(die);
run_depth(Context,
          #try_catch{code_block = Code,
                     catches = Catches,
                     finally = Finally,
                     line = Line},
          false,
    ...
    try
        ...
        Exception when ?IS_OBJECT(Exception) ->
            Ret = lists:foldl(fun (#catch_block{} = CatchBlock, throw) ->
    ...
    run_finally(Context, Finally, Cover);
run_depth(Context, #variable{type = static, name = VarName} = Var, false, _Cover) ->
    ActiveFun = ephp_context:get_active_function(Context),
    RealValue =
        case ephp_context:get_active_real_class(Context) of
            <<>> ->
            ...
            ClassName ->
                ...
                ephp_class:init_static_value(Classes, NS, ClassName, ActiveFun, VarName, undefined)
        end,
...
-spec run_catch(ephp:context_id(),
                catch_block(),
                Exception :: obj_ref(),
                Finally :: statements(),
                Cover :: boolean()) ->
                   throw | flow_status().
run_catch(Context,
          #catch_block{exception = #variable{data_type = Catch} = Var, code_block = CatchCode},
          Exception,
          Finally,
          Cover) ->
    case ephp_data:instance_of(Context, Exception, Catch) of
        true ->
            ...
            try
                ...
                NewException when ?IS_OBJECT(NewException) ->
                    run_finally(Context, Finally, Cover),
                    throw(NewException)
            end;
        false ->
...
-spec run_loop(PrePost :: pre | post,
...
-spec run_foreach(Context :: ephp:context_id(),
                  ...
                  Var :: variable() | ref(),
                  Elements ::
                      variable() | expression() | {variable() | expression(), [{mixed(), mixed()}]},
                  Statements :: [statement()],
                  Cover :: boolean()) ->
                     break() | return() | false.
...
run_foreach(Context, Key, Var, Object, Statements, Cover) when ?IS_OBJECT(Object) ->
    ...
    if Break =/= break andalso not is_tuple(Break) ->
...
-spec run_switch(ephp:context_id(),
                 condition() | default,
                 [switch_case()],
                 Cover :: boolean()) ->
                    {switch_flow(), break() | return() | false}.
run_switch(Context, default, Cases, Cover) ->
    lists:foldl(fun (_SwitchCase, {exit, Return}) ->
                        {exit, Return};
