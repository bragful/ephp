...
-import(ephp_parser, [add_pos/2, new_line/1, remove_spaces/2, inc_pos/1]).
...
st_interface(<<A:8, Rest/binary>>, Parser, #class{name = undefined} = I)
    ...
    {Rest0, Parser0, {NS, Name}} = class_name(<<A:8, Rest/binary>>, Parser, []),
    st_interface(Rest0, Parser0, I#class{name = Name, namespace = NS});
st_interface(<<E:8, X:8, T:8, E:8, N:8, D:8, S:8, SP:8, Rest/binary>>, Parser, Interface)
    when ?OR(E, $E, $e)
         andalso ?OR(X, $X, $x)
         andalso ?OR(T, $T, $t)
         andalso ?OR(N, $N, $n)
...
st_class(<<E:8, X:8, T:8, E:8, N:8, D:8, S:8, SP:8, Rest/binary>>, Parser, Class)
    when ?OR(E, $E, $e)
         andalso ?OR(X, $X, $x)
         andalso ?OR(T, $T, $t)
         andalso ?OR(N, $N, $n)
...
st_interface_content(<<"/*", Rest/binary>>, Parser, Parsed) ->
    ...
    st_interface_content(Rest0, Parser0, Parsed);
st_interface_content(<<P:8, U:8, B:8, L:8, I:8, C:8, SP:8, Rest/binary>>,
                     Parser,
                     Interface)
    when ?OR(P, $P, $p)
         andalso ?OR(U, $U, $u)
...
st_interface_content(<<P:8, R:8, I:8, V:8, A:8, T:8, E:8, SP:8, Rest/binary>>,
    ...
    when ?OR(P, $P, $p)
         ...
         andalso ?OR(E, $E, $e)
         andalso (?IS_SPACE(SP) orelse ?IS_NEWLINE(SP)) ->
    st_interface_content(<<SP:8, Rest/binary>>, private_level(add_pos(Parser, 7)), Interface);
st_interface_content(<<P:8, R:8, O:8, T:8, E:8, C:8, T:8, E:8, D:8, SP:8, Rest/binary>>,
    ...
    when ?OR(P, $P, $p)
         ...
         andalso ?OR(D, $D, $d)
         andalso (?IS_SPACE(SP) orelse ?IS_NEWLINE(SP)) ->
    st_interface_content(<<SP:8, Rest/binary>>,
                         protected_level(add_pos(Parser, 9)),
                         Interface);
st_interface_content(<<S:8, T:8, A:8, T:8, I:8, C:8, SP:8, Rest/binary>>, Parser, Class)
    when ?OR(S, $S, $s)
         andalso ?OR(T, $T, $t)
         andalso ?OR(A, $A, $a)
         andalso ?OR(I, $I, $i)
         andalso ?OR(C, $C, $c)
         andalso (?IS_SPACE(SP) orelse ?IS_NEWLINE(SP)) ->
    st_interface_content(<<SP:8, Rest/binary>>, static_level(add_pos(Parser, 6)), Class);
st_interface_content(<<F:8, U:8, N:8, C:8, T:8, I:8, O:8, N:8, SP:8, Rest/binary>>,
                     #parser{level = Type,
                             access = RawAccess,
                             final = Final} =
                         Parser,
                     #class{methods = Methods} = Interface)
    when ?OR(F, $F, $f)
         andalso ?OR(U, $U, $u)
         andalso ?OR(N, $N, $n)
         andalso ?OR(C, $C, $c)
    ...
    Method =
        #class_method{name = Fun#function.name,
...
st_interface_content(<<C:8, O:8, N:8, S:8, T:8, SP:8, Rest/binary>>,
                     Parser,
                     #class{constants = Constants} = Interface)
    when ?OR(C, $c, $C)
         andalso ?OR(O, $o, $O)
         andalso ?OR(N, $n, $N)
         andalso ?OR(S, $s, $S)
...
st_class_content(<<"/*", Rest/binary>>, Parser, Parsed) ->
    {Rest0, Parser0, _} = ephp_parser:comment_block(Rest, Parser, Parsed),
    st_class_content(Rest0, Parser0, Parsed);
st_class_content(<<P:8, U:8, B:8, L:8, I:8, C:8, SP:8, Rest/binary>>, Parser, Class)
    when ?OR(P, $P, $p)
         andalso ?OR(U, $U, $u)
         andalso ?OR(B, $B, $b)
         andalso ?OR(L, $L, $l)
...
st_class_content(<<P:8, R:8, I:8, V:8, A:8, T:8, E:8, SP:8, Rest/binary>>, Parser, Class)
    when ?OR(P, $P, $p)
         andalso ?OR(R, $R, $r)
         andalso ?OR(I, $I, $i)
         andalso ?OR(V, $V, $v)
         ...
         andalso ?OR(E, $E, $e)
         andalso (?IS_SPACE(SP) orelse ?IS_NEWLINE(SP)) ->
    st_class_content(<<SP:8, Rest/binary>>, private_level(add_pos(Parser, 7)), Class);
st_class_content(<<P:8, R:8, O:8, T:8, E:8, C:8, T:8, E:8, D:8, SP:8, Rest/binary>>,
    ...
    when ?OR(P, $P, $p)
         ...
         andalso ?OR(D, $D, $d)
         andalso (?IS_SPACE(SP) orelse ?IS_NEWLINE(SP)) ->
    st_class_content(<<SP:8, Rest/binary>>, protected_level(add_pos(Parser, 9)), Class);
st_class_content(<<F:8, I:8, N:8, A:8, L:8, SP:8, Rest/binary>>, Parser, Class)
    when ?OR(F, $F, $f)
         andalso ?OR(I, $I, $i)
         andalso ?OR(N, $N, $n)
         andalso ?OR(A, $A, $a)
         andalso ?OR(L, $L, $l)
         andalso (?IS_SPACE(SP) orelse ?IS_NEWLINE(SP)) ->
    st_class_content(<<SP:8, Rest/binary>>, final_level(add_pos(Parser, 5)), Class);
st_class_content(<<S:8, T:8, A:8, T:8, I:8, C:8, SP:8, Rest/binary>>, Parser, Class)
    when ?OR(S, $S, $s)
         andalso ?OR(T, $T, $t)
         andalso ?OR(A, $A, $a)
         andalso ?OR(I, $I, $i)
         andalso ?OR(C, $C, $c)
         andalso (?IS_SPACE(SP) orelse ?IS_NEWLINE(SP)) ->
    st_class_content(<<SP:8, Rest/binary>>, static_level(add_pos(Parser, 6)), Class);
st_class_content(<<V:8, A:8, R:8, SP:8, Rest/binary>>, Parser, Class)
    when ?OR(V, $V, $v)
         andalso ?OR(A, $A, $a)
         andalso ?OR(R, $R, $r)
         andalso (?IS_SPACE(SP) orelse ?IS_NEWLINE(SP)) ->
...
st_class_content(<<"$", _/binary>> = Rest,
    ...
    when Access =/= undefined orelse Type =:= static ->
    Attr =
        case ephp_parser_expr:expression(Rest, Parser, []) of
            {Rest0, Parser0, #assign{variable = #variable{name = VarName}, expression = Expr}} ->
...
st_class_content(<<C:8, O:8, N:8, S:8, T:8, SP:8, Rest/binary>>,
                 Parser,
                 #class{constants = Constants} = Class)
    when ?OR(C, $c, $C)
         andalso ?OR(O, $o, $O)
         andalso ?OR(N, $n, $N)
         andalso ?OR(S, $s, $S)
...
normal_public_level(Parser) ->
    Parser#parser{level = normal,
                  final = false,
                  access = undefined}.

public_level(Parser) ->
    Parser#parser{access = public}.

protected_level(Parser) ->
    Parser#parser{access = protected}.

private_level(Parser) ->
    Parser#parser{access = private}.

static_level(Parser) ->
    Parser#parser{level = static}.

abstract_level(Parser) ->
    Parser#parser{level = abstract}.

final_level(Parser) ->
    Parser#parser{final = true}.
