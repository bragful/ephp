...
empty(_Vars, undefined, _Context) ->
    true;
empty(_Vars, <<"0">>, _Context) ->
    true;
empty(_Vars, <<>>, _Context) ->
    true;
empty(_Vars, false, _Context) ->
    true;
empty(Vars, #variable{} = VarPath, Context) ->
    empty(Vars, search(VarPath, erlang:get(Vars), Context, false), Context);
empty(_Vars, _, _Context) ->
    false.

set_bulk(VarRef, VarVals, Context) ->
    Vars =
        lists:foldl(fun ({VarPath, Value}, Vars) when is_record(VarPath, variable) ->
...
zip_args(VarsSrc, VarsDst, ValArgs, FuncArgs, FunctName, Line, Context) ->
    Zip = fun (#ref{var = VarRef}, [{#variable{} = VarName, _} | RestArgs]) ->
    ...
    lists:foldl(fun (#ref{var = #variable{data_type = DataType}} = Ref,
                     {I, [{_, Value} | _] = Acc})
                        when DataType =/= undefined ->
                        Check(I, DataType, Value, ephp_data:instance_of(Context, Value, DataType)),
                        {I + 1, Zip(Ref, Acc)};
                    (#variable{data_type = DataType} = Var, {I, [{_, Value} | _] = Acc})
                        when DataType =/= undefined ->
                        Check(I, DataType, Value, ephp_data:instance_of(Context, Value, DataType)),
                        {I + 1, Zip(Var, Acc)};
                    (VarOrRef, {I, Acc}) ->
...
search(#variable{name = Root,
       ...
       _Base) ->
    File = ephp_context:get_active_file(Context),
    ephp_error:handle_error(Context, {error, eundefidx, Line, File, ?E_NOTICE, {Root}}),
    undefined;
search(#variable{name = Root,
                 idx = [],
                 line = Line},
       ...
       true) ->
    File = ephp_context:get_active_file(Context),
    ephp_error:handle_error(Context, {error, eundefvar, Line, File, ?E_NOTICE, {Root}}),
    undefined;
...
search(#variable{name = Root, line = Line} = Var, Vars, Context, Base)
    ...
    File = ephp_context:get_active_file(Context),
    ephp_error:handle_error(Context, {error, eillegalstr, Line, File, ?E_WARNING, {Root}}),
...
search(#variable{name = Root, line = Line} = Var, Vars, Context, Base)
    ...
    File = ephp_context:get_active_file(Context),
    ephp_error:handle_error(Context, {error, estrcast, Line, File, ?E_NOTICE, {}}),
...
search(#variable{name = Root,
    ...
    File = ephp_context:get_active_file(Context),
    ephp_error:handle_error(Context, {error, enoinitidx, Line, File, ?E_NOTICE, {Root}}),
    case Idx of
        [] ->
            <<>>;
        [NewRoot | NewIdx] ->
...
search(#variable{name = Root,
    ...
    case byte_size(Vars) =< Root of
        true ->
            File = ephp_context:get_active_file(Context),
            ephp_error:handle_error(Context, {error, eundefidxstr, Line, File, ?E_NOTICE, {Root}}),
            case Idx of
                [] ->
                    <<>>;
                [NewRoot | NewIdx] ->
        ...
        false ->
            ...
            case Idx of
                [] ->
                    NewVars;
                [NewRoot | NewIdx] ->
                    search(Var#variable{name = NewRoot, idx = NewIdx}, NewVars, Context, Base)
            end
    end;
search(#variable{name = Root,
                 idx = [],
...
search(#variable{name = Root,
    ...
    case translate_mem(ephp_array:find(Root, Vars)) of
        {ok, #var_ref{ref = global}} ->
            search(Var#variable{name = NewRoot,
                                idx = Idx,
                                type = array},
                   Vars,
                   Context,
...
change(#variable{name = <<"this">>, idx = [{object, ObjRoot, _Line} | Idx]} = Var,
    ...
    Vars;
%% TODO: check when auto is passed as idx to trigger an error
change(#variable{name = Root,
                 idx = [{object, NewRoot, _Line} | Idx],
                 line = Line},
       Value,
       Vars,
       Context) ->
    ...
    case ephp_array:find(Root, Vars) of
        ...
        {ok, NewVars} when ?IS_ARRAY(NewVars) ->
            ephp_array:store(Root,
                             change(#variable{name = NewRoot, idx = Idx}, Value, NewVars, Ctx),
                             Vars);
        {ok, MemRef} when ?IS_MEM(MemRef) ->
            case ephp_mem:get(MemRef) of
