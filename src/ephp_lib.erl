...
-type php_function_defs() :: php_function() | {php_function(), php_function_opts()}.
-type php_function() :: atom().
-type php_function_opts() :: [php_function_opt()].
-type php_function_opt() ::
    pack_args |
...
-type type_arg() :: mixed | string | integer | array | object | resource | raw | type_ref.
-type validation_arg() :: {type_arg(), default_value()} | type_arg().
...
-export_type([php_function/0, php_function_results/0, php_function_opts/0,
              php_function_opt/0, php_const_results/0, config_section/0, config_param/0,
              php_config_results/0, validation_args/0]).

-callback init_func() -> php_function_results().
-callback init_config() -> php_config_results().
-callback init_const() -> php_const_results().
-callback handle_error(ephp_error:error_type(),
                       ephp_error:error_level(),
                       Args :: term()) ->
                          string() | ignore.
...
maybe_register_errors(Ctx, Module) ->
    case erlang:function_exported(Module, handle_error, 3) of
        true ->
            ephp_error:add_message_handler(Ctx, Module);
        false ->
            ok
    end.
...
register_functions(Ctx, Module) ->
    lists:foreach(fun(Func) -> register_func(Ctx, Module, Func) end, Module:init_func()).
