...
only_text_test_() ->
    [?_assertMatch([#print_text{text = <<"<html><body><h1>Hi world!</h1></body></html>">>}],
     ...
     ?_assertMatch([#print_text{text = <<"hello world!">>}], ?PARSE("hello world!"))].

different_tags_test_() ->
    [?_assertMatch([#eval{statements =
                              [#assign{variable = #variable{name = <<"a">>},
                                       expression = #int{int = 5}}]}],
                   ?PARSE("<? $a = 5; ?>")),
     ?_assertMatch([#print_text{text = <<"5">>}], ?PARSE("<?=5?>")),
     ?_assertMatch([#eval{statements =
                              [#assign{variable = #variable{name = <<"a">>},
                                       expression = #int{int = 5}}]}],
                   ?PARSE("<?php $a = 5; ?>"))].
    ...
    [?_assertMatch([#eval{statements =
     ...
     ?_assertMatch([#eval{statements =
                              [#assign{variable = #variable{name = <<"a">>},
                                       expression = #int{int = 5}}]}],
                   ?PARSE("<?php $a = 5;")),
     ?_assertMatch({error, eparse, {{line, 1}, {column, 6}}, ?E_PARSE, <<"...">>},
                   catch ?PARSE("<?=5 ")),
     ?_assertMatch([#print_text{text = <<"5 Resultado">>}], ?PARSE("<?=5?> Resultado"))].

literal_test_() ->
    [?_assertMatch([#print_text{text = <<"-5">>}], ?PARSE("<?=-5?>")),
     ?_assertMatch([#print_text{text = <<"5">>}], ?PARSE("<?=5.0?>")),
     ?_assertMatch([#print_text{text = <<"-5">>}], ?PARSE("<?= - 5.0 ?>")),
     ?_assertMatch([#print_text{text = <<"hello world!">>}], ?PARSE("<?='hello world!'?>")),
     ?_assertMatch([#print_text{text = <<"hello world!">>}], ?PARSE("<?=\"hello world!\"?>"))].

concat_test_() ->
    [?_assertMatch([#print{expression =
                               #concat{texts =
                                           [#text{text = <<"hello ">>},
                                            #variable{name = <<"name">>},
                                            #text{text = <<".">>},
     ...
     ?_assertMatch([#print_text{text = <<"hello world!">>}],
                   ?PARSE("<?='hello ' . 'world!'?>"))].

arith_test_() ->
    [?_assertMatch([#print_text{text = <<"126">>}], ?PARSE("<?= 1 + 25 * 5 ?>")),
     ?_assertMatch([#print_text{text = <<"26">>}], ?PARSE("<?=1+25?>")),
     ?_assertMatch([#print_text{text = <<"130">>}], ?PARSE("<?= (1 + 25) * 5 ?>"))].
...
var_indexes_test_() ->
    [?_assertMatch([#print{expression =
                               #variable{name = <<"a">>,
                                         idx = [#int{int = 25}, #variable{name = <<"b">>}]}}],
                   ?PARSE("<?=$a[25][$b]?>")),
     ?_assertMatch([#print{expression =
                               #variable{name = <<"a">>,
                                         idx =
...
switch_statement_test_() ->
    [?_assertMatch([#eval{statements =
                              [#switch{condition = #variable{name = <<"a">>},
                                       cases =
...
post_incr_test_() ->
    [?_assertMatch([#eval{statements = [{post_incr, #variable{name = <<"i">>}, _}]}],
                   ?PARSE("<? $i++; ")),
     ?_assertMatch([#print{expression = {post_incr, #variable{name = <<"i">>}, _}}],
                   ?PARSE("<?=$i++?>"))].

post_decr_test_() ->
    [?_assertMatch([#eval{statements = [{post_decr, #variable{name = <<"i">>}, _}]}],
                   ?PARSE("<? $i--; ")),
     ?_assertMatch([#print{expression = {post_decr, #variable{name = <<"i">>}, _}}],
                   ?PARSE("<?=$i--?>"))].

pre_incr_test_() ->
    [?_assertMatch([#eval{statements = [{pre_incr, #variable{name = <<"i">>}, _}]}],
                   ?PARSE("<? ++$i; ")),
     ?_assertMatch([#print{expression = {pre_incr, #variable{name = <<"i">>}, _}}],
                   ?PARSE("<?=++$i?>"))].

pre_decr_test_() ->
    [?_assertMatch([#eval{statements = [{pre_decr, #variable{name = <<"i">>}, _}]}],
                   ?PARSE("<? --$i; ")),
     ?_assertMatch([#print{expression = {pre_decr, #variable{name = <<"i">>}, _}}],
                   ?PARSE("<?=--$i?>"))].

pre_and_post_incr_and_decr_as_index_test_() ->
    [?_assertMatch([#print{expression =
                               #variable{name = <<"a">>,
                                         idx = [{post_incr, #variable{name = <<"i">>}, _}]}}],
                   ?PARSE("<?=$a[$i++]?>")),
     ?_assertMatch([#print{expression =
                               #variable{name = <<"a">>,
                                         idx = [{pre_decr, #variable{name = <<"i">>}, _}]}}],
                   ?PARSE("<?=$a[--$i]?>")),
     ?_assertMatch([#print{expression =
                               #variable{name = <<"a">>,
                                         idx = [{post_decr, #variable{name = <<"i">>}, _}]}}],
                   ?PARSE("<?=$a[$i--]?>")),
     ?_assertMatch([#print{expression =
                               #variable{name = <<"a">>,
                                         idx = [{pre_incr, #variable{name = <<"i">>}, _}]}}],
                   ?PARSE("<?=$a[++$i]?>"))].

not_test_() ->
    [?_assertMatch([#print{expression =
                               #if_block{conditions = {operation_not, #variable{name = <<"a">>}, _},
     ...
     ?_assertMatch([#eval{statements =
                              [#assign{variable = #variable{name = <<"a">>},
                                       expression =
                                           {operation_not, #variable{name = <<"a">>}, _}}]}],
                   ?PARSE("<? $a = !$a; ?>")),
     ?_assertMatch([#print{expression = {operation_not, #variable{name = <<"a">>}, _}}],
                   ?PARSE("<?=!$a?>"))].
...
function_call_test_() ->
    [?_assertMatch([#eval{statements =
     ...
     ?_assertMatch([#print{expression = #call{name = <<"myfunc">>}}],
                   ?PARSE("<?=myfunc()?>"))].

comments_test_() ->
    [?_assertMatch([], ?PARSE("<? # comment ?>")),
     ?_assertMatch([], ?PARSE("<? // comment ")),
     ?_assertMatch([], ?PARSE("<? /* comment */ ")),
     ?_assertMatch([#print_text{text = <<"Result = ">>},
...
try_catch_test_() ->
    [?_assertMatch([#eval{statements =
     ...
     ?_assertMatch([#eval{statements =
                              [#try_catch{code_block = [#constant{name = <<"OK">>}],
                                          catches =
                                              [#catch_block{exception = #variable{name = <<"e">>},
                                                            code_block =
                                                                [#constant{name = <<"FAIL">>}]}],
                                          finally = []}]}],
