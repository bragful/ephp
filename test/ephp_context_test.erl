...
set_and_get_array_test(Ctx) ->
    [?_assertEqual(ok,
                   ephp_context:set(Ctx, #variable{name = <<"a">>, idx = [#int{int = 0}]}, 10)),
     ?_assertEqual(ok,
                   ephp_context:set(Ctx, #variable{name = <<"a">>, idx = [#int{int = 1}]}, 20)),
     ?_assertEqual(ok,
                   ephp_context:set(Ctx,
                                    #variable{name = <<"a">>, idx = [#text{text = <<"hello">>}]},
                                    30)),
     ?_assertEqual(ok,
                   ephp_context:set(Ctx,
                                    #variable{name = <<"a">>, idx = [#int{int = 2}, #int{int = 0}]},
                                    40)),
...
resolve_assign(Ctx) ->
    [?_assertEqual(10,
                   ephp_context:solve(Ctx,
                                      #assign{variable = #variable{name = <<"a">>},
                                              expression = #int{int = 10}}))].

operation_test(Ctx) ->
    [?_assertEqual(10,
                   ephp_context:solve(Ctx,
                                      #operation{type = <<"+">>,
                                                 expression_left = #int{int = 5},
...
global_test(Ctx) ->
    ...
    [?_assertEqual(<<"hello">>,
                   ephp_context:solve(Ctx,
                                      #assign{variable = Var,
                                              expression = #text{text = <<"hello">>}})),
     ?_assertEqual(undefined, ephp_context:solve(SubCtx, {global, [Var], Line})),
     ?_assertEqual(10,
                   ephp_context:solve(SubCtx,
                                      #assign{variable = Var, expression = #int{int = 10}})),
     ?_assertEqual(10,
                   ephp_mem:get(
                       ephp_context:get(SubCtx, #variable{name = <<"a">>}))),
     ?_assertEqual(10,
                   ephp_mem:get(
                       ephp_context:get(Ctx, #variable{name = <<"a">>})))].
